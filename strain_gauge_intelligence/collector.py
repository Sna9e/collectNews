...
def _collect_type(
    ...
    for window_days in windows:
        ...
        exa_settings = {
            "search_type": "auto",
            "category": "news" if item_type == "news" else "",
            "content_mode": "highlights_text",
            "highlights_max_characters": 1200,
            "text_max_characters": 2000,
            "start_published_date": (now - _dt.timedelta(days=int(window_days))).isoformat().replace("+00:00", "Z"),
            "end_published_date": (now + _dt.timedelta(days=1)).isoformat().replace("+00:00", "Z"),
        }
        for query_record in queries:
            ...
            batch, _ = filter_blocked_search_results(batch, manual_domains=blocked_domains)
        ...
        if item_type == "patent":
            ...
            if len(all_items) < min_count:
                for query_record in queries[: max(1, int(max_queries_per_type or 4))]:
                    ...
                    patent_batch, _ = filter_blocked_search_results(
                        patent_batch,
                        manual_domains=blocked_domains,
                    )
...
def collect_strain_gauge_module(
    ...
    all_raw = []
    news, raw, news_windows = _collect_type(
        "news", config, provider, tavily_key, exa_key, max_queries_per_type,
        results_per_query, generated_at, search_fn, manual_blocked_domains,
    )
    all_raw.extend(raw)
    patents, raw, patent_windows = _collect_type(
        "patent", config, provider, tavily_key, exa_key, max_queries_per_type,
        results_per_query, generated_at, search_fn, manual_blocked_domains,
    )
    all_raw.extend(raw)
    papers, raw, paper_windows = _collect_type(
        "paper", config, provider, tavily_key, exa_key, max_queries_per_type,
        results_per_query, generated_at, search_fn, manual_blocked_domains,
    )
    all_raw.extend(raw)
