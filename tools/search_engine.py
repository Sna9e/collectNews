...
_CONSUMER_DAILY_IMPORTANCE_TERMS = [
    ...
    "订单", "产能", "芯片", "影像", "屏幕", "电池", "快充", "散热", "折叠", "AI", "端侧AI",
...
def build_consumer_daily_queries(topic_pack, query_suffix="", max_queries=6):
    ...
    candidates = []
    for query in topic_pack.get("queries", []) or []:
...
def _consumer_daily_result_score(result, topic_pack):
    ...
    for company in topic_pack.get("companies", []) or []:
        if company and str(company).lower() in blob:
            score += 2.1
...
def rank_consumer_daily_results(results, topic_pack, limit=None):
    ranked = []
    for index, raw_item in enumerate(results or []):
        item = dict(raw_item or {})
        if "search_provider" not in item:
...
def search_consumer_daily_tavily(
    ...
    queries = build_consumer_daily_queries(topic_pack, query_suffix=query_suffix, max_queries=max_queries)
    if not queries:
        return []

    settings = {
        "include_text": "中国 国内 国产 新品 参数 发布 供应链 硬件 量产 AI 智能驾驶 折叠屏",
        "exclude_text": "lawsuit court copyright privacy security stock 股价 股票 诉讼 版权 隐私 安全漏洞 macos",
    }
    merged_results = []
    seen_urls = set()

    def append_results(batch):
    ...
    for query_index, query in enumerate(queries):
        domain_batch = _search_web_tavily(
            query,
            merged_sites_text,
            timelimit,
        ...
        append_results(domain_batch)

        if query_index < broad_query_count:
            broad_query = f"{query} 中国 国内 中文 科技 媒体 重要".strip()
            broad_batch = _search_web_tavily(
                broad_query,
                "",
                timelimit,
                max_results=max(8, min(12, max_results_per_query)),
                tavily_key=tavily_key,
                exa_settings=settings,
            )
            append_results(broad_batch)

    return rank_consumer_daily_results(merged_results, topic_pack, limit=160)
