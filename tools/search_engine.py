...
def _build_tavily_query(query, settings):
    ...
    include_terms = _split_search_filter_terms(settings.get("include_text"))
    exclude_terms = _split_search_filter_terms(settings.get("exclude_text"))
...
def _count_filter_hits(text, terms):
    ...
    return sum(1 for term in terms or [] if term and term in haystack)
...
def _rank_tavily_results(results, settings):
    ...
    for index, raw_item in enumerate(results or []):
        ...
        rank_score = base_score + include_hits * 0.08 - exclude_hits * 0.18 + freshness_bonus
        ranked.append(
            {
                "item": item,
                "score": rank_score,
...
def _search_web_tavily(query, sites_text, timelimit, max_results=20, tavily_key="", exa_settings=None):
    ...
    try:
        ...
        results = _rank_tavily_results(resp.get("results", []), settings)
