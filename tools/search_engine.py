...
_EXA_LIVECRAWL_VALUES = {"never", "fallback", "preferred", "always"}
...
def _search_web_exa(query, sites_text, timelimit, max_results=20, exa_key="", exa_settings=None):
    ...
    livecrawl = str(settings.get("livecrawl") or "fallback").strip().lower()
    if livecrawl in _EXA_LIVECRAWL_VALUES:
