...
_GLOBAL_TRUSTED_SOURCE_DOMAINS = [
    "reuters.com",
    "bloomberg.com",
    "cnbc.com",
    "wsj.com",
...
_SOURCE_QUALITY_CONFIG_PATH = Path(__file__).resolve().with_name("source_quality_domains.json")
...
def _load_source_quality_config():
    default_payload = {
        "low_trust_domains": list(_LOW_TRUST_SOURCE_DOMAINS),
        "trusted_domains_extra": [],
    }
    try:
        if not _SOURCE_QUALITY_CONFIG_PATH.exists():
            return default_payload
        payload = json.loads(_SOURCE_QUALITY_CONFIG_PATH.read_text(encoding="utf-8"))
        if not isinstance(payload, dict):
            return default_payload
        merged = dict(default_payload)
        for key in ("low_trust_domains", "trusted_domains_extra"):
            value = payload.get(key, default_payload.get(key, []))
            if isinstance(value, list):
                merged[key] = [str(item or "").strip().lower() for item in value if str(item or "").strip()]
        return merged
    except Exception as exc:
        print(f"⚠️ Failed to load source quality config: {exc}")
        return default_payload
...
def _coerce_domain_list(values):
    if isinstance(values, str):
        return parse_sites_text(values)
    merged = []
    seen = set()
...
def is_low_trust_source_domain(host):
    config = _load_source_quality_config()
    return _domain_in_allowlist(str(host or "").strip().lower(), config.get("low_trust_domains", _LOW_TRUST_SOURCE_DOMAINS))


def is_trusted_source_domain(host, trusted_domains=None):
    config = _load_source_quality_config()
    merged_domains = _coerce_domain_list(_GLOBAL_TRUSTED_SOURCE_DOMAINS)
    merged_domains.extend(_coerce_domain_list(CN_DOMAIN_PRESET))
    merged_domains.extend(_coerce_domain_list(config.get("trusted_domains_extra", [])))
    merged_domains.extend(_coerce_domain_list(trusted_domains))
    return _domain_in_allowlist(str(host or "").strip().lower(), merged_domains)


def filter_results_for_source_quality(results, trusted_domains=None, strict_unknown=False):
    filtered = []
    dropped = []
    stats = {
    ...
    for item in results or []:
        ...
        enriched["_source_host"] = host

        if is_low_trust_source_domain(host):
            stats["dropped_low_trust_count"] += 1
            if len(dropped) < 5:
                dropped.append(f"已屏蔽低可信来源：{host or 'unknown'} | {enriched.get('title', '未命名结果')}")
            continue

        trusted = is_trusted_source_domain(host, trusted_domains=trusted_domains)
        enriched["source_quality"] = "trusted" if trusted else "unknown"

        if strict_unknown and not trusted:
            stats["dropped_unknown_count"] += 1
            if len(dropped) < 5:
                dropped.append(f"已过滤未知来源：{host or 'unknown'} | {enriched.get('title', '未命名结果')}")
            continue

        filtered.append(enriched)

    stats["kept_count"] = len(filtered)
    return filtered, stats, dropped
...
def _normalize_tavily_filter_terms(value):
    raw = str(value or "").replace("，", ",").replace("；", ";").strip().lower()
    if not raw:
        return []
...
def _filter_results_by_text_controls(results, include_terms=None, exclude_terms=None):
    include_terms = [term for term in include_terms or [] if term]
    exclude_terms = [term for term in exclude_terms or [] if term]
...
def _search_web_tavily(query, sites_text, timelimit, max_results=20, tavily_key="", tavily_settings=None):
    if not tavily_key:
        return []
    sites = parse_sites_text(sites_text)
    settings = dict(tavily_settings or {})
    search_depth = str(settings.get("search_depth") or "advanced").strip().lower()
    if search_depth not in _TAVILY_SEARCH_DEPTHS:
        search_depth = "advanced"

    topic = str(settings.get("topic") or "news").strip().lower()
    if topic not in _TAVILY_TOPICS:
        topic = "news"

    configured_results = int(settings.get("num_results") or max_results or 10)
    resolved_results = max(1, min(20, min(max_results or configured_results, configured_results)))
    chunks_per_source = max(1, min(3, int(settings.get("chunks_per_source") or 3)))
    auto_parameters = bool(settings.get("auto_parameters"))
    include_answer = bool(settings.get("include_answer"))
    raw_content_mode = str(settings.get("include_raw_content") or "off").strip().lower()
    include_terms = _normalize_tavily_filter_terms(settings.get("include_text"))
    exclude_terms = _normalize_tavily_filter_terms(settings.get("exclude_text"))
    try:
        ...
        if raw_content_mode == "markdown":
            payload["include_raw_content"] = "markdown"
        elif raw_content_mode == "text":
        ...
        results = _dedupe_search_results(resp.get("results", []))
        results, _, _ = filter_results_for_source_quality(results)
        results = _filter_results_by_text_controls(results, include_terms=include_terms, exclude_terms=exclude_terms)
...
def search_web(
    ...
    if resolved_provider == "hybrid":
        ...
        merged = _filter_results_by_text_controls(
            merged,
            include_terms=_normalize_tavily_filter_terms((tavily_settings or {}).get("include_text")),
            exclude_terms=_normalize_tavily_filter_terms((tavily_settings or {}).get("exclude_text")),
        )
        return merged
