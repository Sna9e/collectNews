...
_HTML_META_TAG_RE = re.compile(r"<meta\b[^>]*>", re.IGNORECASE | re.DOTALL)
_HTML_ATTR_RE = re.compile(r"([:\w-]+)\s*=\s*(['\"])(.*?)\2", re.IGNORECASE | re.DOTALL)
_JSONLD_PUBLISHED_RE = re.compile(
    r"['\"](?:datePublished|dateCreated)['\"]\s*:\s*['\"]([^'\"]+)['\"]",
    re.IGNORECASE,
)
_URL_DATE_RES = (
    re.compile(r"(?:^|/)(20\d{2})/(0?[1-9]|1[0-2])/(0?[1-9]|[12]\d|3[01])(?:/|$)"),
    re.compile(r"(?:^|[/_-])(20\d{2})-(0?[1-9]|1[0-2])-(0?[1-9]|[12]\d|3[01])(?:[/_.-]|$)"),
    re.compile(r"(?:^|/)(20\d{2})(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])(?:/|$)"),
)
_LABELED_DATE_RE = re.compile(
    r"(?:date\s+published|published\s+(?:on|time)|publication\s+date|发布时间|发布日期|发布于|公开日)"
    r"\s*[:：]?\s*"
    r"(20\d{2}[年/-]\d{1,2}[月/-]\d{1,2}(?:日)?(?:[ T]\d{1,2}:\d{2}(?::\d{2})?(?:\s*(?:Z|[+-]\d{2}:?\d{2}))?)?"
...
def _record_blocked_search_results(blocked_results):
    if not blocked_results:
        return
    with _SEARCH_DIAGNOSTICS_LOCK:
        stats = _SEARCH_DIAGNOSTICS["source_blocking"]
        for item in blocked_results:
            stats["blocked_count"] += 1
            domain = str(item.get("domain") or "unknown").strip().lower() or "unknown"
            category = str(item.get("category") or "unknown").strip().lower() or "unknown"
            stats["by_domain"][domain] = int(stats["by_domain"].get(domain, 0) or 0) + 1
            stats["by_category"][category] = int(stats["by_category"].get(category, 0) or 0) + 1
...
def _filter_results_by_source_policy(results, blocked_domains=None):
    kept, blocked = filter_blocked_search_results(results, manual_domains=blocked_domains)
    _record_blocked_search_results(blocked)
    return kept
...
def _build_tavily_payloads(query, sites, timelimit, max_results, tavily_key, settings):
    ...
    blocked_domains = get_effective_blocked_domains(settings.get("_manual_blocked_domains"))
    excluded_domains = list(dict.fromkeys(_TAVILY_DEFAULT_EXCLUDE_DOMAINS + blocked_domains))[:150]
    if excluded_domains:
        base_payload["exclude_domains"] = excluded_domains
...
def filter_results_to_local_day(results, target_date, tzinfo=None, allow_text_date=True):
    ...
    for item in results or []:
        ...
        published_dt, used_key, date_details = resolve_result_publication_datetime(item)
        if published_dt:
            if date_details.get("conflict"):
                stats["date_conflict_count"] += 1
            ...
            enriched_item["publication_date_confidence"] = date_details.get("confidence", "")
            enriched_item["publication_date_conflict"] = bool(date_details.get("conflict"))
...
def search_consumer_daily(
    ...
    manual_blocked_domains = parse_blocked_domains(blocked_domains)
    if manual_blocked_domains:
        tavily_settings["_manual_blocked_domains"] = manual_blocked_domains
        exa_runtime_settings["_manual_blocked_domains"] = manual_blocked_domains
    merged_results = []
    seen_urls = set()
    ...
    def append_results(batch, query_text=""):
        filtered_batch = _filter_results_by_source_policy(batch, blocked_domains=manual_blocked_domains)
...
def _search_web_exa(query, sites_text, timelimit, max_results=20, exa_key="", exa_settings=None):
    ...
    blocked_domains = get_effective_blocked_domains(settings.get("_manual_blocked_domains"))
    if blocked_domains and category not in _EXA_CATEGORY_BLOCKS_DATE_FILTERS:
        payload["excludeDomains"] = blocked_domains[:1200]
    ...
    if category not in _EXA_CATEGORY_BLOCKS_DATE_FILTERS:
        start_published = str(settings.get("start_published_date") or "").strip()
        end_published = str(settings.get("end_published_date") or "").strip()
        if not start_published and not end_published:
...
def search_web(
    ...
    manual_blocked_domains = parse_blocked_domains(blocked_domains)
    runtime_settings = dict(exa_settings or {})
    if manual_blocked_domains:
        runtime_settings["_manual_blocked_domains"] = manual_blocked_domains

    if resolved_provider == "exa":
        results = _search_web_exa(
            query,
            sites_text,
            timelimit,
            max_results=max_results,
            exa_key=exa_key,
            exa_settings=runtime_settings,
        )
        return _filter_results_by_source_policy(results, blocked_domains=manual_blocked_domains)
    ...
    if resolved_provider == "hybrid":
        ...
        return _filter_results_by_source_policy(
            _dedupe_search_results(merged),
...
def _extract_labeled_date_candidates(text, source_prefix="content"):
    candidates = []
    searchable = html.unescape(str(text or ""))[:12000]
    for index, match in enumerate(_LABELED_DATE_RE.finditer(searchable)):
        parsed = _coerce_datetime(match.group(1))
        if parsed:
            candidates.append((parsed, f"{source_prefix}:labeled_date_{index + 1}", 82))
    return candidates


def _extract_html_date_candidates(raw_html, source_prefix="page_html"):
    raw = str(raw_html or "")
    if not raw:
        return []

    candidates = []
    for index, match in enumerate(_JSONLD_PUBLISHED_RE.finditer(raw)):
        parsed = _coerce_datetime(match.group(1))
        if parsed:
            candidates.append((parsed, f"{source_prefix}:jsonld_datePublished_{index + 1}", 120))

    for index, tag in enumerate(_HTML_META_TAG_RE.findall(raw)):
        attrs = {
            key.lower(): html.unescape(value).strip()
            for key, _, value in _HTML_ATTR_RE.findall(tag)
        }
        marker = str(
            attrs.get("property")
            or attrs.get("name")
        ...
        if marker not in _PAGE_DATE_META_KEYS:
            continue
        parsed = _coerce_datetime(attrs.get("content") or attrs.get("datetime"))
        if parsed:
            candidates.append((parsed, f"{source_prefix}:meta:{marker}:{index + 1}", 115))

    visible_head = _TAG_RE.sub(" ", raw[:20000])
    candidates.extend(_extract_labeled_date_candidates(visible_head, source_prefix=source_prefix))
    return candidates


def _extract_url_date_candidate(url):
    try:
    ...
    for pattern in _URL_DATE_RES:
        ...
        try:
            ...
            return parsed, "url_path_date", 90
        except ValueError:
            continue
    return None


def _normalize_page_date_evidence(value):
    if not value:
        return None
    if isinstance(value, dict):
        parsed = _coerce_datetime(
            value.get("published_at")
            or value.get("published_date")
            or value.get("datetime")
            or value.get("date")
        )
        source_key = str(value.get("source_key") or "page_fetch").strip()
    elif isinstance(value, (list, tuple)) and value:
        parsed = _coerce_datetime(value[0])
        source_key = str(value[1] if len(value) > 1 else "page_fetch").strip()
    else:
        parsed = _coerce_datetime(value)
        source_key = "page_fetch"
    if not parsed:
        return None
    return parsed, source_key or "page_fetch", 125


def resolve_result_publication_datetime(result, page_date_evidence=None):
    """Resolve publication time from page evidence before provider timestamps."""
    item = dict(result or {})
    evidence = []

    normalized_page_evidence = _normalize_page_date_evidence(page_date_evidence)
    if normalized_page_evidence:
        evidence.append(normalized_page_evidence)

    metadata = item.get("metadata") if isinstance(item.get("metadata"), dict) else {}
    for key in ("datePublished", "date_published", "article_published_time", "page_published_at"):
        parsed = _coerce_datetime(item.get(key) or metadata.get(key))
        if parsed:
            evidence.append((parsed, key, 110))

    raw_content = str(item.get("raw_content") or "")
    if raw_content:
        evidence.extend(_extract_html_date_candidates(raw_content, source_prefix="raw_content"))

    url_candidate = _extract_url_date_candidate(item.get("url") or item.get("source_url"))
    if url_candidate:
        evidence.append(url_candidate)

    lead_text = " ".join(
        str(item.get(key) or "") for key in ("title", "content", "snippet", "description")
    )[:6000]
    evidence.extend(_extract_labeled_date_candidates(lead_text, source_prefix="result_text"))

    for key in _DATE_KEY_CANDIDATES:
        parsed = _coerce_datetime(item.get(key))
        if parsed:
            evidence.append((parsed, key, 50))
    provider_date = _coerce_datetime(item.get("provider_published_date"))
    if provider_date:
        evidence.append((provider_date, "provider_published_date", 50))
    ...
    deduped = []
    seen = set()
    for parsed, source_key, priority in evidence:
        normalized = parsed.astimezone(datetime.timezone.utc)
        marker = (source_key, normalized.isoformat())
        if marker in seen:
            continue
        seen.add(marker)
        deduped.append((normalized, source_key, priority))

    chosen = max(deduped, key=lambda row: row[2])
    conflict_keys = []
    for parsed, source_key, _ in deduped:
        if source_key == chosen[1]:
            continue
        if abs((parsed - chosen[0]).total_seconds()) > 48 * 3600:
            conflict_keys.append(source_key)

    confidence = "verified_page" if chosen[2] >= 100 else ("corroborated" if chosen[2] >= 80 else "provider")
    details = {
        "confidence": confidence,
        "conflict": bool(conflict_keys),
        "conflicting_keys": sorted(set(conflict_keys)),
        "evidence_count": len(deduped),
        "evidence": [
            {"source_key": source_key, "published_at": parsed.isoformat(), "priority": priority}
            for parsed, source_key, priority in sorted(deduped, key=lambda row: row[2], reverse=True)[:8]
        ],
    }
    return chosen[0], chosen[1], details


def _is_safe_public_http_url(url, resolve_dns=False):
    try:
    ...
    try:
        ...
        if not resolved_addresses:
            return False
        try:
            return all(ipaddress.ip_address(value).is_global for value in resolved_addresses)
        except ValueError:
            return False
    return address.is_global


def fetch_page_publication_datetime(url, timeout=8):
    """Fetch public page metadata only; no authentication or access-control bypass."""
    ...
    try:
        with urllib.request.urlopen(request, timeout=timeout) as response:
            ...
            if not _is_safe_public_http_url(final_url, resolve_dns=True):
                return None
    ...
    candidates = _extract_html_date_candidates(decoded, source_prefix="fetched_page")
    if not candidates:
        return None
    parsed, source_key, _ = max(candidates, key=lambda row: row[2])
    return {"published_at": parsed.isoformat(), "source_key": source_key}


def _fetch_page_date_evidence_for_results(results, max_page_checks=18, page_date_fetcher=None):
    fetcher = page_date_fetcher or fetch_page_publication_datetime
    candidates = []
    for index, item in enumerate(results or []):
        if len(candidates) >= max(int(max_page_checks or 0), 0):
            break
        url = str((item or {}).get("url") or (item or {}).get("source_url") or "").strip()
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(6, len(candidates))) as executor:
        futures = {executor.submit(fetcher, url): index for index, url in candidates}
        for future in concurrent.futures.as_completed(futures):
            try:
                normalized = _normalize_page_date_evidence(future.result())
            except Exception:
                normalized = None
            if normalized:
                evidence_by_index[futures[future]] = normalized
    return evidence_by_index, len(candidates)


def extract_result_datetime(result):
    parsed, source_key, _ = resolve_result_publication_datetime(result)
    return parsed, source_key



def audit_recent_news_results(
    results,
    now=None,
    max_age_hours=30,
    future_tolerance_hours=6,
    enabled=True,
    verify_page_dates=False,
    max_page_checks=18,
    page_date_fetcher=None,
):
    baseline = now or datetime.datetime.now(datetime.timezone.utc)
    ...
    stats = {
        ...
        "date_conflict_count": 0,
        "dropped_timestamp_conflict_count": 0,
        "provider_timestamp_only_count": 0,
        "page_date_checked_count": 0,
        "page_date_verified_count": 0,
    ...
    page_evidence_by_index = {}
    if verify_page_dates:
        page_evidence_by_index, checked_count = _fetch_page_date_evidence_for_results(
            results,
            max_page_checks=max_page_checks,
            page_date_fetcher=page_date_fetcher,
        )
        stats["page_date_checked_count"] = checked_count
        stats["page_date_verified_count"] = len(page_evidence_by_index)

    for index, item in enumerate(results or []):
        stats["audited_count"] += 1
        published_dt, used_key, date_details = resolve_result_publication_datetime(
            item,
            page_date_evidence=page_evidence_by_index.get(index),
        )
        if not published_dt:
            stats["dropped_missing_timestamp_count"] += 1
            if len(dropped_samples) < 5:
        ...
        if date_details.get("confidence") == "provider":
            stats["provider_timestamp_only_count"] += 1
        if date_details.get("conflict"):
            stats["date_conflict_count"] += 1
        ...
        enriched_item["publication_date_confidence"] = date_details.get("confidence", "")
        enriched_item["publication_date_conflict"] = bool(date_details.get("conflict"))
        enriched_item["publication_date_evidence"] = list(date_details.get("evidence", []))
        enriched_item["age_hours"] = round(age_hours, 2)

        if delta > max_age_delta:
            stats["dropped_stale_count"] += 1
            if date_details.get("conflict"):
                stats["dropped_timestamp_conflict_count"] += 1
            if len(dropped_samples) < 5:
                dropped_samples.append(
                    f"超出窗口 {round(age_hours, 1)}h（依据 {used_key}）：{item.get('title', '未命名结果')}"
                )
            continue
    ...
    if stats["date_conflict_count"] or stats["page_date_checked_count"]:
        warnings.append(
            "发布时间复核："
            f"网页检查 {stats['page_date_checked_count']} 条，提取到页面日期 {stats['page_date_verified_count']} 条，"
            f"发现时间证据冲突 {stats['date_conflict_count']} 条，"
            f"其中因页面/URL 证据超窗或异常剔除 {stats['dropped_timestamp_conflict_count']} 条。"
        )
    if dropped_samples:
        warnings.append("样例：" + "；".join(dropped_samples))
...
def verify_selected_news_by_title_search(
    ...
    for item in news_items or []:
        ...
        for query in queries:
            ...
            batch = _filter_results_by_source_policy(batch, blocked_domains=blocked_domains)
            for result in batch:
        ...
        fresh_results, freshness_stats, _ = audit_recent_news_results(
            ...
            future_tolerance_hours=6,
            enabled=True,
            verify_page_dates=(search_fn is None or page_date_fetcher is not None),
            max_page_checks=max_results_per_query,
            page_date_fetcher=page_date_fetcher,
        )
