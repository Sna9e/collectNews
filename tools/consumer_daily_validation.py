...
def load_source_registry(registry_path=None):
    ...
    for key in ("official", "high_quality_cn", "medium_quality_cn", "low_quality_or_aggregator"):
...
def build_verification_queries(event, topic_pack, target_date, limit=6):
    ...
    if any(term.lower() in event.normalized_title.lower() for term in ("rokid", "xreal", "ray-ban", "meta", "glasses")):
        queries.append(f"{base} AI glasses new feature today")
...
def build_verified_topic_events(
    ...
    discovery_candidates, rejected = _build_candidates(discovery_results, topic_pack, target_date, resolved_window, registry)
    ...
    if verification_search_fn:
        ...
        for event in initial_events[:max_initial_events]:
            for query in build_verification_queries(event, topic_pack, target_date, limit=verification_queries_per_event):
                try:
                    batch = verification_search_fn(query, topic_pack) or []
                except Exception:
                    batch = []
                for result in batch:
                    ...
                    verification_results.append(enriched)

    all_candidates, rejected2 = _build_candidates(
        list(discovery_results or []) + verification_results,
        topic_pack,
        target_date,
        resolved_window,
        registry,
    ...
    final_events = sorted(
        cluster_articles_into_events(all_candidates, topic_pack, target_date, resolved_window, registry=registry),
        key=lambda item: (item.confidence_level == "confirmed", item.confidence_level == "likely", item.confidence_score, item.independent_source_count),
        reverse=True,
    )
    ...
    rejected_summary.extend(rejected[:20])
    warnings = []
    if not confirmed and not likely:
        warnings.append("今日可确认新闻不足：该专题没有达到 confirmed/likely 门槛的事件。")
...
def build_verified_news_package(topic_verified_events, target_date, time_window):
    target = target_date.isoformat() if isinstance(target_date, datetime.date) else str(target_date)[:10]
    return VerifiedNewsPackage(target_date=target, time_window=str(time_window), topics=list(topic_verified_events or []))
...
def event_blueprints_from_verified_topic(topic_verified, limit=8):
    events = (topic_verified.confirmed_events + topic_verified.likely_events)[:limit]
    blueprints = []
    for event in events:
        primary = event.evidence_articles[0] if event.evidence_articles else None
        blueprints.append(
...
def raw_results_from_verified_topic(topic_verified):
    rows = []
    for event in topic_verified.confirmed_events + topic_verified.likely_events:
...
def validate_consumer_daily_quality(package, min_events_per_topic=1):
    ...
    for topic in topics:
        formal = topic.confirmed_events + topic.likely_events
        ...
        for rejected in topic.rejected_summary:
            ...
            elif rejected.confidence_level == "weak":
                weak_count += 1
            else:
                rejected_count += 1
        for event in formal:
