...
class GistMemoryManager:
    ...
    def get_event_bank_summary(self, topic: str, limit: int = 8) -> str:
        with self._lock:
    ...
    def get_topic_context(self, topic: str) -> str:
        history_text = self.get_topic_history(topic)
        event_text = self.get_event_bank_summary(topic)
        return f"【历史观察摘要】\n{history_text}\n\n【历史事件图谱】\n{event_text}"
