...
def generate_ppt(data, timeline_data, filename, model_name):
    ...
    title_box.text_frame.paragraphs[0].text = "《FPC-RD 科技资讯》"
    ...
    for section in data:
        ...
        if finance.get("is_public"):
            ...
            if not data_available:
                ...
                metrics = [
                    ...
                    f"▪ 成交量: {finance.get('volume', 'N/A')}",
                    f"▪ 10日均量: {finance.get('avg_volume_10d', 'N/A')}",
                    f"▪ 成交额: {finance.get('turnover_value', 'N/A')}",
                    f"▪ 分析师评级: {finance.get('analyst_rating_summary', 'N/A')}",
                    f"▪ 目标价: {finance.get('analyst_price_targets', 'N/A')}",
                ]
            ...
            if analyst_firm_views:
                analyst_box = finance_slide.shapes.add_textbox(Inches(4.5), Inches(6.05), Inches(5.0), Inches(0.65))
                analyst_tf = analyst_box.text_frame
                analyst_tf.word_wrap = True
                analyst_title = analyst_tf.paragraphs[0]
                analyst_title.text = "🏦 大行观点"
                analyst_title.font.size = Pt(11)
                analyst_title.font.bold = True
                analyst_title.font.color.rgb = RGBColor(0, 51, 102)
                analyst_body = analyst_tf.add_paragraph()
                analyst_body.text = "；".join(analyst_firm_views[:3])
                analyst_body.font.size = Pt(10)
                analyst_body.space_before = Pt(4)
        ...
        for news in news_items:
            ...
            fitted_entries = _fit_entries_to_single_page(entries, max_units=31, chars_per_line=36)

            slide = prs.slides.add_slide(prs.slide_layouts[1] if len(prs.slide_layouts) > 1 else prs.slide_layouts[0])
            clear_placeholders(slide)

            title_box = slide.shapes.add_textbox(Inches(0.45), Inches(0.58), Inches(9.15), Inches(0.82))
            title_text = _shorten(_get(news, "title", "未命名情报"), 60)
            title_box.text_frame.paragraphs[0].text = title_text
            title_box.text_frame.paragraphs[0].font.size = Pt(22)
            title_box.text_frame.paragraphs[0].font.bold = True

            body_box = slide.shapes.add_textbox(Inches(0.45), Inches(1.35), Inches(9.15), Inches(5.95))
