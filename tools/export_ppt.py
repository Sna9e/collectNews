...
def _is_consumer_daily_deck(data):
    sections = [section for section in data or [] if _get(section, "data", [])]
    if not sections:
        return False
    ...
    for offset in range(0, len(news_items), chunk_size):
        ...
        for local_idx, news in enumerate(chunk, start=offset + 1):
            ...
            meta = (
                ...
                f"时间: {_get(news, 'date_check', '近期')}  |  来源: {_shorten(_clean_display_text(_get(news, 'source', '未知来源')), 18)}"
            )
...
def generate_ppt(data, timeline_data, filename, model_name):
    ...
    sections_with_news = [section for section in data if _get(section, "data", [])]
    is_consumer_daily = _is_consumer_daily_deck(sections_with_news)
    ...
    for section_index, section in enumerate(sections_with_news, start=1):
        news_items = _get(section, "data", [])
        if not news_items:
            continue

        if len(sections_with_news) > 1:
            _add_section_divider_slide(prs, section, section_index, len(sections_with_news))
