...
def _build_news_entries(news, extraction_stats, section_warnings):
    entries = [
        _make_entry(
    ...
    if event_id:
        entries.append(_make_entry(f"🧷 事件ID: {event_id}", font_size=11, color=(100, 100, 100), space_after=6))
    ...
    for ref in timeline_refs[:2]:
        entries.append(
            _make_entry(
                f"⏱️ 承接时间线: [{_get(ref, 'date', '近期')}] {_shorten(_clean_display_text(_get(ref, 'event', '')), 28)}",
                font_size=11,
                bold=True,
                color=(192, 102, 0),
                space_after=3,
        ...
        entries.append(
            _make_entry(
                f"原因: {_shorten(_clean_display_text(_get(ref, 'reason', '')), 95)}",
                font_size=10,
                color=(124, 45, 18),
                space_after=6,
            )
...
def _add_chart_slide(prs, news, chart_info):
    slide = prs.slides.add_slide(prs.slide_layouts[1] if len(prs.slide_layouts) > 1 else prs.slide_layouts[0])
    clear_placeholders(slide)

    title_box = slide.shapes.add_textbox(Inches(0.5), Inches(0.5), Inches(9), Inches(0.8))
    title_box.text_frame.paragraphs[0].text = f"{_shorten(_get(news, 'title', '未命名情报'), 36)}（图表）"
    ...
    meta_p.text = f"来源: {_get(news, 'source', '未知来源')} | 时间: {_get(news, 'date_check', '近期')}"
    ...
    if chart_path and os.path.exists(chart_path):
        slide.shapes.add_picture(chart_path, Inches(0.75), Inches(1.45), width=Inches(8.2))

    note_box = slide.shapes.add_textbox(Inches(0.7), Inches(6.4), Inches(8.4), Inches(0.5))
    note_p = note_box.text_frame.paragraphs[0]
    note_p.text = f"图表说明: {_shorten(_clean_display_text(_get(chart_info, 'chart_title', '') or _get(news, 'title', '数据对比')), 80)}"
...
def generate_ppt(data, timeline_data, filename, model_name):
    ...
    for section in data:
        ...
        for news in news_items:
            ...
            entries = _build_news_entries(news, extraction_stats, section_warnings)
            entry_pages = _paginate_entries(entries, max_units=26)

            for page_idx, entry_page in enumerate(entry_pages):
                slide = prs.slides.add_slide(prs.slide_layouts[1] if len(prs.slide_layouts) > 1 else prs.slide_layouts[0])
                clear_placeholders(slide)

                title_box = slide.shapes.add_textbox(Inches(0.5), Inches(0.6), Inches(9), Inches(0.8))
                title_text = _get(news, "title", "未命名情报")
                if page_idx > 0:
                    title_text = f"{_shorten(title_text, 38)}（续{page_idx + 1}）"
                title_box.text_frame.paragraphs[0].text = title_text
                title_box.text_frame.paragraphs[0].font.size = Pt(22)
                title_box.text_frame.paragraphs[0].font.bold = True

                body_box = slide.shapes.add_textbox(Inches(0.5), Inches(1.4), Inches(9.0), Inches(5.6))
                _render_entries(body_box.text_frame, entry_page)

            if has_chart:
                _add_chart_slide(prs, news, chart_info)
