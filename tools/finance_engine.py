...
def _format_price_targets(mean_price, high_price, low_price, currency):
    mean_value = _safe_float(mean_price)
    high_value = _safe_float(high_price)
    low_value = _safe_float(low_price)
    if mean_value is None and high_value is None and low_value is None:
        return "N/A"
    parts = []
    if mean_value is not None:
        parts.append(f"一致目标价 {mean_value:.2f} {currency}")
    if high_value is not None and low_value is not None:
        parts.append(f"区间 {low_value:.2f}-{high_value:.2f} {currency}")
    return " | ".join(parts) if parts else "N/A"


def _extract_recent_broker_views(ticker):
    if yf is None or ticker is None:
        return []
    try:
        upgrades = getattr(ticker, "upgrades_downgrades", None)
        if upgrades is None or getattr(upgrades, "empty", True):
            return []
        recent_rows = upgrades.tail(40)
        broker_views = []
        seen = set()
        for _, row in recent_rows.iterrows():
            ...
            if not any(keyword in firm_lower for keyword in ANALYST_FIRM_KEYWORDS):
                continue
            to_grade = str(row.get("ToGrade", "") or "").strip()
            action = str(row.get("Action", "") or "").strip()
            snippet = " ".join(part for part in [firm, action, to_grade] if part).strip()
            key = snippet.lower()
            if not snippet or key in seen:
                continue
            broker_views.append(snippet)
            seen.add(key)
            if len(broker_views) >= 3:
                break
        return broker_views
    except Exception:
        return []
...
def _build_analyst_snapshot(info, ticker=None, currency="USD"):
    recommendation = str(info.get("recommendationKey", "") or "").strip()
    ...
    if recommendation:
        ...
        recommendation_text = recommendation_map.get(recommendation.lower(), recommendation)
        if analyst_count:
            recommendation_text = f"{recommendation_text}（{int(float(analyst_count))}家覆盖）"

    price_targets = _format_price_targets(
        info.get("targetMeanPrice"),
        info.get("targetHighPrice"),
        info.get("targetLowPrice"),
        currency,
    )
    broker_views = _extract_recent_broker_views(ticker)
    return {
        "analyst_rating_summary": recommendation_text,
        "analyst_price_targets": price_targets,
        "analyst_firm_views": broker_views,
    }


def _supplement_analyst_snapshot(ticker_code, payload):
    if yf is None or not ticker_code or not isinstance(payload, dict):
        return payload
    needs_snapshot = (
        payload.get("analyst_rating_summary") in (None, "", "N/A")
        or payload.get("analyst_price_targets") in (None, "", "N/A")
        or not payload.get("analyst_firm_views")
    )
    ...
    try:
        ...
        if not info:
            return payload
        snapshot = _build_analyst_snapshot(info, ticker=ticker, currency=payload.get("currency") or info.get("currency", "USD"))
        if snapshot.get("analyst_rating_summary") not in (None, "", "N/A"):
            payload["analyst_rating_summary"] = snapshot["analyst_rating_summary"]
        if snapshot.get("analyst_price_targets") not in (None, "", "N/A"):
            payload["analyst_price_targets"] = snapshot["analyst_price_targets"]
        if snapshot.get("analyst_firm_views"):
            payload["analyst_firm_views"] = snapshot["analyst_firm_views"]
    except Exception:
        pass
    return payload
...
def fetch_from_yfinance(ticker_code):
    ...
    try:
        with warnings.catch_warnings():
            ...
            avg_volume_10d = None
            if hist_df is not None and not hist_df.empty and "Volume" in hist_df:
                avg_volume_10d = hist_df["Volume"].tail(10).mean()
            turnover_value = None
            if current_price is not None and _safe_float(volume) is not None:
                turnover_value = current_price * float(volume)

            low_52w = info.get("fiftyTwoWeekLow", "N/A")
            high_52w = info.get("fiftyTwoWeekHigh", "N/A")
            currency = info.get("currency", "USD")
            analyst_snapshot = _build_analyst_snapshot(info, ticker=ticker, currency=currency)
            ...
            return {
                ...
                "avg_volume_10d": format_number(avg_volume_10d),
                "turnover_value": format_number(turnover_value),
                "analyst_rating_summary": analyst_snapshot["analyst_rating_summary"],
                "analyst_price_targets": analyst_snapshot["analyst_price_targets"],
                "analyst_firm_views": analyst_snapshot["analyst_firm_views"],
    ...
    try:
        ...
        for row in k_list:
            if not isinstance(row, (list, tuple)) or len(row) < 6:
                continue
            open_value = _safe_float(row[1])
            close_value = _safe_float(row[2])
            high_value = _safe_float(row[3])
            low_value = _safe_float(row[4])
            volume_value = _safe_float(row[5])
            trade_date = pd.to_datetime(row[0], errors="coerce")
            if trade_date is None or (pd is not None and pd.isna(trade_date)):
                continue
            if None in (open_value, close_value, high_value, low_value, volume_value):
                continue
            df_data.append({
                "Date": trade_date,
        ...
        raw_volume = _safe_float(parts[36])
        volume = None if raw_volume is None else (raw_volume * 100 if not t_sym.startswith('us') else raw_volume)
        avg_volume_10d = hist_df["Volume"].tail(10).mean() if hist_df is not None and not hist_df.empty else None
        turnover_value = current_price * volume if current_price is not None and volume is not None else None
        market_cap_raw = _safe_float(parts[45])
        market_cap = (market_cap_raw * 100000000) if market_cap_raw is not None else 0
        ...
        return {
            ...
            "avg_volume_10d": format_number(avg_volume_10d),
            "turnover_value": format_number(turnover_value),
            "analyst_rating_summary": "N/A",
            "analyst_price_targets": "N/A",
            "analyst_firm_views": [],
            "chart_path": chart_path
        }
    except Exception as e:
...
def fetch_from_xueqiu(ticker_code):
    ...
    try:
        ...
        erp = f"{((1 / pe) - 0.042) * 100:.2f}%" if pe and pe > 0 else "N/A"
        avg_volume_10d = hist_df["Volume"].tail(10).mean() if hist_df is not None and not hist_df.empty else None
        turnover_value = quote.get('current', 0) * quote.get('volume', 0) if quote.get('current') and quote.get('volume') else None
        chart_path = generate_pro_kline_chart(ticker_code, hist_df, f"kline_{ticker_code}.png")
...
def fetch_financial_data(ai_driver, company_name):
    ...
    for name, fn in source_chain:
        ...
        data = fn(ticker_code)
        if data:
            return _supplement_analyst_snapshot(ticker_code, data)
    ...
    return {
        ...
        "avg_volume_10d": "N/A", "turnover_value": "N/A",
        "analyst_rating_summary": "N/A", "analyst_price_targets": "N/A", "analyst_firm_views": [],
        "chart_path": None,
    }
