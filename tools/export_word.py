...
def _resolve_report_title(data, timeline_data):
    ...
    if len(scope_keys) == 1:
        ...
        if only_key == "weekly_7d":
            return "DeepSeek 企业级科技周报"
        if only_key == "monthly_30d":
            return "DeepSeek 企业级科技月度观察"
    return "DeepSeek 企业级深度科技研报"
...
def generate_word(data, timeline_data, filename, model_name):
    ...
    title = doc.add_heading(_resolve_report_title(data, timeline_data), 0)
    ...
    for section in data:
        ...
        if summary_title and overall_insight:
            ...
            p_overall = doc.add_paragraph(overall_insight)
            p_overall.paragraph_format.line_spacing = 1.4
