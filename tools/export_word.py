...
def _format_timeline_event_summary(item):
    summary = str(_get(item, 'event_summary', '') or '').strip()
    return summary
...
def generate_word(data, timeline_data, filename, model_name):
    ...
    title = doc.add_heading("AI 企业级深度科技研报", 0)
    ...
    if timeline_data:
        ...
        for t_data in timeline_data:
            is_company_tracking_timeline = _get(t_data, 'report_style', '') == 'company_tracking'
            ...
            if not events:
                ...
                empty_run = p_empty.add_run("暂无有效时间线。")
            ...
            for item in events:
                ...
                if _get(item, 'appears_in_later_news', False):
                    ...
                    if highlight_level == 'key':
                        ...
                        badge_text = "【重点详细新闻】 "
                        badge_color = RGBColor(192, 102, 0)
                    else:
                        event_run.font.color.rgb = RGBColor(31, 78, 121)
                        badge_text = "【已对应详细新闻】 "
                        badge_color = RGBColor(31, 78, 121)
                ...
                event_summary = _format_timeline_event_summary(item)
                if is_company_tracking_timeline and event_summary:
                    p_summary = doc.add_paragraph()
                    p_summary.paragraph_format.left_indent = Pt(24)
                    summary_run = p_summary.add_run(event_summary)
                ...
                if _get(item, 'appears_in_later_news', False):
                    ...
                    if matched_title:
                        ...
                        title_run = p_title.add_run(f"对应{detail_label}：")
                        title_run.bold = True
                        title_run.font.color.rgb = (
                            RGBColor(192, 102, 0)
                            if str(_get(item, 'highlight_level', '') or '').strip().lower() == 'key'
                            else RGBColor(31, 78, 121)
                        )
                        p_title.add_run(matched_title)
                    if match_reason and not is_company_tracking_timeline:
    ...
    for section in data:
        is_company_tracking_section = _get(section, 'report_style', '') == 'company_tracking'
        ...
        if not news_items:
            p_empty = doc.add_paragraph()
            empty_run = p_empty.add_run("    在指定时间范围内，未发现符合标准的重大情报。")
            empty_run.italic = True
            continue

        for news in news_items:
            detail_index = int(_get(news, 'detail_index', 0) or 0)
            detail_prefix = f"详细新闻 {detail_index}｜" if is_company_tracking_section and detail_index else ""
            doc.add_heading(f"🔹 {detail_prefix}{_get(news, 'title', '未命名情报')}", level=2)
            ...
            if event_id_text and not is_company_tracking_section:
            ...
            if timeline_refs:
                ...
                for ref in timeline_refs:
                    ...
                    if reason and not is_company_tracking_section:
