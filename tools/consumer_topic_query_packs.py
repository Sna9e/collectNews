...
CONSUMER_TOPIC_DEFAULT_LIMIT = 80
CONSUMER_TOPIC_RESULTS_PER_QUERY = 8

_NOISE_TERMS = [
    "推荐", "相关推荐", "大家都在看", "热门文章", "广告", "促销", "优惠券", "壁纸", "教程",
...
def build_consumer_topic_queries_from_pack(pack, query_suffix="", max_queries=None):
    ...
    if suffix:
        queries = [f"{query} {suffix}" if _contains_cjk(query) else query for query in queries]
...
def rank_results_by_consumer_topic_pack(results, topic_pack, limit=CONSUMER_TOPIC_DEFAULT_LIMIT):
    ...
    for row in scored:
        ...
        if row["score"] < -1.5:
            continue
        if any(_result_similarity(row["item"], picked["item"]) >= 0.86 for picked in selected):
            continue
    ...
    if len(selected) < min(limit, 20):
        for row in scored:
            if len(selected) >= limit:
                break
            if any(_result_similarity(row["item"], picked["item"]) >= 0.9 for picked in selected):
                continue
            selected.append(row)
...
def collect_consumer_topic_search_results(
    ...
    depth_limits = {"light": 10, "normal": 16, "wide": 28}
    max_queries = depth_limits.get(str(search_depth or "wide").lower(), 28)
