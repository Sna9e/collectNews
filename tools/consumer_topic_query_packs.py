...
from tools.source_blocklist import filter_blocked_search_results, parse_blocked_domains


CONSUMER_TOPIC_DEFAULT_LIMIT = 120
CONSUMER_TOPIC_RESULTS_PER_QUERY = 6

CHANNEL3_MAJOR_NEWS_TOPIC_IDS = (
    "consumer_phone",
    "ar_vr_ai_glasses",
    "ev_smart_car",
    "ai_weekly",
...
CONSUMER_TOPIC_QUERY_PACKS = {
    "consumer_phone": ConsumerTopicQueryPack(
        ...
        topic_name="消费电子 / 手机与新型显示",
        description="追踪中国国内手机、平板、PC、可穿戴、折叠屏及消费电子供应链的重大新品、关键参数、系统更新、量产和市场变化，海外事件仅保留对国内产业具有直接参考价值的重大进展。",
        aliases=["手机", "智能手机", "消费电子", "手机产业", "平板电脑", "AI PC", "智能穿戴", "折叠屏", "新型显示", "smartphone", "mobile phone", "consumer electronics"],
        core_entities=["手机", "智能手机", "消费电子", "平板电脑", "AI PC", "智能穿戴", "折叠屏", "新型显示", "AI手机", "端侧AI"],
        ...
        product_terms=["Mate", "Pura", "Nova", "Xiaomi", "Redmi", "iQOO", "Find", "Reno", "OnePlus", "Ace", "Magic", "Galaxy S", "Galaxy Z", "iPhone", "Pixel", "Xperia", "平板", "笔记本", "智能手表", "智能耳机", "Android", "iOS", "HarmonyOS", "HyperOS", "ColorOS", "OriginOS", "MagicOS"],
        technology_terms=["SoC", "芯片", "骁龙", "天玑", "麒麟", "A系列芯片", "端侧AI", "AI手机", "AI PC", "大模型", "影像", "CMOS", "潜望长焦", "OLED", "LTPO", "柔性OLED", "折叠屏", "铰链", "UTG", "Micro OLED", "MicroLED", "屏幕", "电池", "快充", "散热", "卫星通信", "Wi-Fi 7", "UWB", "OTA", "系统更新", "预售", "开售", "销量", "价格"],
        ...
        queries=[
            "本周 中国 消费电子 重大新品 发布",
            "今日 中国 消费电子 新品 发布",
            "今日 手机 平板 笔记本 智能穿戴 新品",
            "今日 折叠屏 OLED 面板 量产 供应链",
            "今日 手机 新品 参数 发布",
        ...
        prompt_focus="请只保留设定时间窗内中国国内消费电子领域真实发生的重大事件，覆盖手机、平板、PC、可穿戴、折叠屏和关键显示/零部件供应链。优先正式发布、量产、交付、明确参数变化和重大市场动作；不要把旧款评测、促销软文、配件广告、历史回顾或无信息增量的小更新当作新闻。",
    ),
    "ar_vr_ai_glasses": ConsumerTopicQueryPack(
        ...
        domains=["ithome.com", "leikeji.com", "ifanr.com", "zhidx.com", "qbitai.com", "jiqizhixin.com", "mydrivers.com", "uploadvr.com", "roadtovr.com", "theverge.com"],
    ...
    "robotics_embodied_ai": ConsumerTopicQueryPack(
        ...
        domains=["ithome.com", "jiqizhixin.com", "qbitai.com", "zhidx.com", "leiphone.com", "ofweek.com", "elecfans.com", "gongkong.com", "therobotreport.com", "roboticsbusinessreview.com", "ieee.org", "techcrunch.com", "theverge.com"],
...
def get_channel3_major_news_topic_query_packs():
    """Return the five independent topic packs used by channel 3."""
    return [deepcopy(CONSUMER_TOPIC_QUERY_PACKS[topic_id]) for topic_id in CHANNEL3_MAJOR_NEWS_TOPIC_IDS]
...
def build_consumer_topic_focus_hint(pack):
    ...
    pieces = [
        ...
        "频道三是中国消费科技重大新闻简报，覆盖消费电子、智能眼镜、智能汽车、AI和机器人五类专题，不是公司金融分析。",
        "必须优先保留所选24小时或7天内真实发生的产品发布、关键参数变化、技术路线、供应链、量产、价格、销量、渠道和公司动作。",
...
def rank_results_by_consumer_topic_pack(results, topic_pack, limit=CONSUMER_TOPIC_DEFAULT_LIMIT):
    ...
    for idx, item in enumerate(results or []):
        ...
        domestic_title_hits = _count_hits(title_l, payload.get("domestic_entities", []))
        domestic_blob_hits = _count_hits(blob, payload.get("domestic_entities", []))
        global_title_hits = _count_hits(title_l, payload.get("global_entities", []))
        china_context_hits = _count_hits(blob, ["中国", "国内", "国产", "本土", "在华", "中国市场"])
        score += domestic_title_hits * 5.0
        score += global_title_hits * 1.5
        ...
        if host.endswith(".cn"):
            score += 1.8
        elif _contains_cjk(f"{title} {content}"):
            score += 0.8
        if payload.get("china_focus", True):
            score += china_context_hits * 1.2
            if global_title_hits and not (domestic_title_hits or domestic_blob_hits or china_context_hits or host.endswith(".cn")):
                score -= 2.0
...
def collect_consumer_topic_search_results(
    ...
    depth_limits = {"light": 6, "normal": 10, "wide": 16}
    max_queries = depth_limits.get(str(search_depth or "normal").lower(), 10)
    ...
    for query in queries:
        ...
        if manual_blocked_domains:
            search_kwargs["blocked_domains"] = manual_blocked_domains
        batch = search_web(
            query,
            "",
            timelimit,
        ...
        batch, _ = filter_blocked_search_results(batch, manual_domains=manual_blocked_domains)
...
def filter_consumer_results_by_freshness(
    results,
    topic_pack,
    lookback_window,
    current_dt=None,
    ...
    filtered, stats, warnings = audit_recent_news_results(
        ...
        enabled=True,
        verify_page_dates=verify_page_dates,
    ...
    if str(lookback_window or "").lower() == "7d":
        stats["window_label"] = "最近7天（向前7天并容忍未来6小时的发布时间误差）"
    else:
        stats["window_label"] = "最近24小时（向前30小时并容忍未来6小时的发布时间误差）"
