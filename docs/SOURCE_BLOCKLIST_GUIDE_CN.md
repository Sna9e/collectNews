...
`www.36kr.com` 会归一化为 `36kr.com`；`http://www.xix.ai/?lang=en` 会归一化为 `xix.ai`。36Kr 同时已从重点检索网站和来源白名单移除；代码中仅保留转载识别规则，用于阻止历史数据或其他站点的 36Kr 搬运稿进入正式新闻。

截至 2026-09-04，本地永久名单已经保存用户指定的 9 个规范域名：`bitrss.com`、`dev.to`、`vocus.cc`、`jethrojeff.com`、`36kr.com`、`thevergetoday.pages.dev`、`custommapposter.com`、`villadaba.com`、`xix.ai`。这些域名同时进入内置硬屏蔽，因此即使本地永久配置文件丢失，新运行仍会在供应商请求前排除并在结果返回后复检。
...
域名屏蔽之后，频道一最近 24 小时结果和频道三排名靠前的候选还会从以下位置提取发布时间：
...
频道三每专题最多抽查 8 个公开页面，并分别显示“最近24小时”或“最近7天”的真实窗口标签。页面日期核验状态会继续透传到事件门禁；只有已登记的官方/专业来源、事实字段完整且具有中国公司或中国市场事实时，单篇页面核验结果才可能进入 `likely`。`.cn` 域名、中文转述和页面日期本身都不足以证明事件属于中国国内重大新闻。前瞻、发布日历和多源重复传闻仍会剔除。
