...
- 执行 PWG `daily_scan`。
- 固定复用 Exa 搜索；不调用 Tavily。
- 可设置 query 数、每条 query 结果数、回溯天数和报告日期。
- 可选择是否写入 `data/pwg_intelligence/pwg_intelligence.xlsx`。
- 一键生成：
  - raw JSON
  - raw Excel
  - PWG Excel 数据库
...
- 页面会显示 Raw 结果数、过滤后数量、分类评分数量、人工复核数量和周报机会数量。
- 页面提供上述文件下载按钮。
...
| 文件类型 | 路径 |
| --- | --- |
| raw JSON / raw Excel | `data/pwg_intelligence/raw/` |
| PWG Excel 数据库 | `data/pwg_intelligence/pwg_intelligence.xlsx` |
| 日报 / 周报 | `data/pwg_intelligence/reports/` |
...
- 前端入口当前不新增大模型调用；历史 DeepSeek 离线质量复核结果仅作为归档保留，当前运行接口已停用 DeepSeek。
- 报告层会把英文 Exa 摘要转换为中文事实句展示，但不会改写 raw JSON / Excel 中的原始摘要字段。
- 复杂英文标题中的公司名、产品名和专有技术术语会保留英文。
- 当前 Excel 写入仍是第一版逻辑：重建 DEMO 骨架 + 本轮入选行，尚未做历史正式数据增量合并。

## 14. 公共信息源屏蔽

PWG collector 继续复用公共 `search_web()`，因此自动启用 `tools/source_blocklist.json` 中的域名规则和机器人自动生成内容标记。Streamlit 侧栏 `信息源屏蔽` 中新增的域名会显式传入 PWG 采集器；命令行运行时也可通过环境变量 `NEWS_BLOCKED_DOMAINS` 提供逗号分隔名单。

该门禁发生在 PWG 自身的 URL 规范化、相关性分类、来源等级和机会评分之前。基于历史 raw JSON 重建日报/周报时，报告写入层也会重新检查当前内置和手动名单，但不会删除或改写原始 JSON。它只删除明确命中的站点或自动内容，不改变 PWG 的 A-D 来源等级规则，也不会把被屏蔽结果作为 D 级线索重新补回。完整操作见 `docs/SOURCE_BLOCKLIST_GUIDE_CN.md`。

## 15. 2026-08-26 调试记录

- 命令行现在和 Streamlit 一样，会从 `.streamlit/secrets.toml` 或环境变量读取 Exa/Tavily Key；Key 不会写入输出。
- Windows 控制台输出采用 ASCII 转义 JSON，避免结果正文包含 `‑` 等 GBK 不支持字符时，在文件已经写完后仍因 `UnicodeEncodeError` 返回失败。JSON/XLSX/Markdown 文件继续使用 UTF-8 中文。
- 2-query 真实烟测得到 raw 6 条，3 条超窗、3 条无关，最终 0 条；系统写入明确 warnings，没有用低质量线索补齐。
- 10-query 真实烟测得到 raw 40、过滤后 14、分类 13；分类规则、来源等级、成熟度和评分理由覆盖率均为 100%，但 13 条全部为 C 级并需要人工复核。
- 日报 `validation_outputs/channel_debug/pwg_full/reports/PWG_daily_brief_2026-08-26.md` 与周报 `PWG_weekly_review_2026-W35.md` 均生成成功。周报机会数为 0，符合当前证据强度。
- 当前主要风险不是运行失败，而是 CPO/光纤/通用高折射率材料对 PWG 关键词的假阳性。后续应增强“polymer waveguide + 产品/结构/应用”的共现门槛，并增加公司官网、标准、专利和论文原文的 A/B 级定向 query。
