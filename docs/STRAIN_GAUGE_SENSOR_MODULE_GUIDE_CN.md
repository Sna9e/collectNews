...
本专题复用公共 `search_web()`，自动应用 `tools/source_blocklist.json` 中的域名屏蔽和机器人自动生成内容标记。Streamlit 侧栏新增的手动域名会传入新闻、专利和论文检索；Google Patents 兜底结果也会执行同一份本地域名检查。

命令行运行时可通过 `NEWS_BLOCKED_DOMAINS` 环境变量增加临时规则。门禁只处理明确命中的域名和自动内容声明，不改变原有相关性、字段完整性和数量校验逻辑。完整操作见 `docs/SOURCE_BLOCKLIST_GUIDE_CN.md`。

## 9. 2026-08-26 长时间窗修复与真实烟测

- 旧实现把 365/1095/1825 天窗口映射为 Exa 的 `timelimit="m"`，供应商只召回约 30 天内容，随后本地再按 1-5 年过滤，导致专利和论文天然召回不足。
- 当前会向 Exa 明确传递 `startPublishedDate` 和 `endPublishedDate`：专利优先 365 天、不足扩展至 1095 天；论文优先 1095 天、不足扩展至 1825 天。
- 专利和论文普通网页检索不再强制套用 Exa `research paper` 类别；新闻仍使用 `news` 类别。
- 轻量真实烟测使用每类 2 个 query、每 query 3 条结果，原始结果为新闻 6、专利 12、论文 12；最终保留新闻 2、专利 0、论文 1。
- 输出位于 `validation_outputs/channel_debug/strain/`。数量校验为未通过，报告明确列出专利和论文不足，没有静默跳过或用低相关内容补齐。
- 论文从修复前同等轻量烟测的 0 条提高到 1 条，说明长窗口和普通网页模式有效；专利仍为 0，根因是普通 Exa 结果字段不完整且 Google Patents XHR 可能返回 503。后续必须接入稳定专利 API/许可数据源，不能绕过站点访问控制。
