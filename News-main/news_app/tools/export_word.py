...
def generate_word(data, timeline_data, filename, model_name):
    ...
    title = doc.add_heading("DeepSeek 企业级科技研报", 0)
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER

    meta_p = doc.add_paragraph()
    meta_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    meta_run = meta_p.add_run(
        f"生成日期: {datetime.date.today()}  |  数据来源: Tavily  |  模型: {model_name}"
    )
    ...
    for section in data:
        ...
        if not section["data"]:
            doc.add_paragraph("在指定时间范围内未发现符合标准的情报。").font.italic = True
            continue
