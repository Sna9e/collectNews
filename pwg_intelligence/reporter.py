...
REPORT_TECH_TERM_PATTERNS = (
    r"Micro LED",
    r"co-packaged optics",
    r"\bCPO\b",
    r"\bOBO\b",
    r"\bNPO\b",
    r"polymer optical waveguide",
    r"flexible polymer waveguide",
    r"optical circuit board",
    r"optical wiring board",
    r"optical engine",
    r"optical interposer",
    r"optical backplane",
    r"optical RDL",
    r"PIC coupling",
    r"\bPMT\b",
...
def _extract_report_fact_points(value):
    text = _clean_summary_text(value, max_chars=900)
    terms = []
    for pattern in REPORT_TECH_TERM_PATTERNS:
    ...
    for match in re.findall(
        r"(?:USD|US\$|\$)\s*[\d,.]+\s*(?:million|billion|mn|bn|M|B)?|\b\d+(?:\.\d+)?\s*(?:Gbps|Tbps|GHz|nm|um|μm|mm|%|million|billion)\b|\b20\d{2}\b",
        text,
        flags=re.IGNORECASE,
    ):
        item = match.strip()
        if item and item.lower() not in {existing.lower() for existing in numbers}:
            numbers.append(item)
    points = []
    if terms:
        points.append(f"技术或产品关键词包括{'、'.join(terms[:5])}")
    if numbers:
        points.append(f"明确披露的数字包括{'、'.join(numbers[:5])}")
    return "；".join(points)


def _report_summary_text(row, max_chars=420):
    summary = _clean_summary_text(row.get("factual_summary"), max_chars=max_chars)
    if summary and _has_cjk(summary):
        return summary

    title = _clean_report_title(row.get("title"))
    source = _clean_text(row.get("source_name")) or _domain_from_url(row.get("source_url"))
    published = _local_date(row.get("published_date")) or _local_date(row.get("event_date")) or _local_date(row.get("collected_at"))
    category = CATEGORY_LABELS.get(_clean_text(row.get("pwg_category")), _clean_text(row.get("main_track")) or "PWG技术情报")
    application = _clean_text(row.get("application_scene"))
    fact_points = _extract_report_fact_points(summary)

    lead_parts = []
    if published:
        lead_parts.append(published.isoformat())
    if source:
        lead_parts.append(source)
    lead = "，".join(lead_parts)
    sentences = []
    if lead and title:
        sentences.append(f"{lead}发布或报道了“{title}”。")
    elif title:
        sentences.append(f"该线索围绕“{title}”展开。")

    if category and application:
        sentences.append(f"该事项归入{category}方向，涉及{application}。")
    elif category:
        sentences.append(f"该事项归入{category}方向。")

    if fact_points:
        sentences.append(f"{fact_points}。")

    source_level = _source_level_text(row)
    maturity = _clean_text(row.get("maturity_level"))
    if source_level or maturity:
        sentences.append(f"当前规则判定来源等级为{source_level or '待定'}、成熟度为{maturity or '待定'}，适合纳入样品规格、合作对象和应用窗口的后续验证清单。")
...
def _render_clue(row):
    return (
        ...
        f"- 事实摘要：{_report_summary_text(row)}\n"
...
def _filter_report_source_rows(rows, blocked_domains=None):
    filtered, _ = filter_blocked_search_results(rows, manual_domains=blocked_domains)
    return list(filtered)


def load_classified_rows_from_json(path, blocked_domains=None):
    with Path(path).open("r", encoding="utf-8") as file_obj:
        payload = json.load(file_obj)
    return _filter_report_source_rows(payload.get("classified_rows") or [], blocked_domains=blocked_domains)
...
def load_recent_classified_rows(raw_dir=DEFAULT_RAW_DIR, end_date=None, days=7, blocked_domains=None):
    target = _report_date_from_value(end_date)
    ...
    return _filter_report_source_rows(rows, blocked_domains=blocked_domains)
...
def write_daily_brief(
    ...
    target = _report_date_from_value(report_date)
    rows = _filter_report_source_rows(rows, blocked_domains=blocked_domains)
