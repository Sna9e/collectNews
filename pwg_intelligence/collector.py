...
def collect_pwg_daily_scan(
    ...
    if not dry_run:
        if provider_key == "exa" and not exa_key:
            raise RuntimeError("EXA_API_KEY is required for PWG Exa collection.")
        if provider_key == "tavily" and not tavily_key:
            raise RuntimeError("TAVILY_API_KEY is required for PWG Tavily collection.")
        if provider_key == "hybrid" and not (exa_key or tavily_key):
            raise RuntimeError("PWG hybrid collection requires EXA_API_KEY or TAVILY_API_KEY.")
    ...
    warnings = []
    if not raw_results:
        warnings.append("搜索供应商未返回原始结果；请检查密钥、查询和供应商状态。")
    elif not records:
        warnings.append("原始结果已返回，但全部因时间超窗、主题不相关或重复被过滤；未用低质量线索凑数。")
    payload = {
        "mode": mode_key,
        "dry_run": False,
        "generated_at": fetched_at,
...
def main(argv=None):
    ...
    parser.add_argument("--exa-key", default=_load_key_from_local_secrets("EXA_API_KEY"))
    parser.add_argument("--tavily-key", default=_load_key_from_local_secrets("TAVILY_API_KEY"))
    ...
    # Keep the Windows CLI portable even when the active console code page is GBK.
