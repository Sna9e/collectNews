...
_EVENT_SUMMARY_TARGET_MAX = 180
_EVENT_SUMMARY_TARGET_SENTENCES = 4
...
_EVENT_SUMMARY_ACTION_RE = re.compile(
    r"(发布|推出|宣布|更新|升级|上线|开放|签署|合作|投资|收购|融资|量产|交付|部署|"
    r"申请|获批|调整|扩大|缩减|上调|下调|测试|launch|release|announce|update|"
    r"deploy|ship|partner|invest|acquire|approve)",
    re.IGNORECASE,
)
_EVENT_SUMMARY_CONCRETE_RE = re.compile(
    r"(\d|%|美元|亿元|万台|版本|型号|参数|价格|客户|市场|工厂|产能|订单|供应链|"
    r"开发者|用户|地区|国家|季度|月|年)",
    re.IGNORECASE,
)
...
class EventDraft(BaseModel):
    ...
    event_summary: str = Field(default="", description="100到180字中文短新闻摘要，通常3到4句，说明主体、动作、对象、关键细节和直接影响，不得编造。")
...
def _event_summary_sentence_score(sentence, event_dict, position=0):
    sentence_text = str(sentence or "").strip()
    if not sentence_text or not _has_substantial_chinese(sentence_text):
        return -1.0

    event_dict = event_dict or {}
    event_text = str(event_dict.get("event", "") or "")
    keywords = [str(item or "").strip() for item in event_dict.get("keywords", []) or []]
    reference_tokens = _tokenize(f"{event_text} {' '.join(keywords)}")
    sentence_tokens = _tokenize(sentence_text)
    shared = reference_tokens & sentence_tokens
    overlap = len(shared) / max(min(len(reference_tokens), len(sentence_tokens)) or 1, 1)
    keyword_hits = sum(1 for keyword in keywords if keyword and keyword.lower() in sentence_text.lower())
    if reference_tokens and len(shared) < 2 and keyword_hits == 0:
        return max(0.0, 0.24 - max(int(position), 0) * 0.04)
    action_bonus = 0.55 if _EVENT_SUMMARY_ACTION_RE.search(sentence_text) else 0.0
    concrete_bonus = 0.35 if _EVENT_SUMMARY_CONCRETE_RE.search(sentence_text) else 0.0
    position_bonus = max(0.0, 0.24 - max(int(position), 0) * 0.04)
    return overlap * 3.2 + min(keyword_hits * 0.35, 1.05) + action_bonus + concrete_bonus + position_bonus


def _select_event_summary_material(text, event_dict, max_sentences=_EVENT_SUMMARY_TARGET_SENTENCES):
    cleaned = _clean_event_summary_text(text)
    if not cleaned:
        return ""

    candidates = []
    for index, raw_sentence in enumerate(_EVENT_SUMMARY_SENTENCE_RE.findall(cleaned)):
        sentence = _ensure_sentence_punctuation(raw_sentence.strip())
        if not sentence or _EVENT_SUMMARY_PATCH_RE.match(sentence):
            continue
        if len(_CJK_RE.findall(sentence)) < 10:
            continue
        score = _event_summary_sentence_score(sentence, event_dict, index)
        if score < 0:
            continue
        candidates.append((index, score, sentence))

    if not candidates:
        return ""

    anchor_indexes = [item[0] for item in candidates if item[1] >= 0.35]
    if not anchor_indexes:
        return ""

    eligible = []
    for index, score, sentence in candidates:
        follows_anchor = any(anchor <= index <= anchor + 3 for anchor in anchor_indexes)
        if score < 0.35 and not follows_anchor:
            continue
        contextual_score = score + (0.30 if follows_anchor and score < 0.35 else 0.0)
        eligible.append((index, contextual_score, sentence))

    ranked = sorted(eligible, key=lambda item: (item[1], -item[0]), reverse=True)

    selected_indexes = {item[0] for item in ranked[:max_sentences]}
    selected = [item[2] for item in eligible if item[0] in selected_indexes]
...
def _build_event_summary_from_result(result, event_dict, min_chars=_EVENT_SUMMARY_TARGET_MIN, max_chars=_EVENT_SUMMARY_TARGET_MAX):
    ...
    for candidate in source_candidates:
        selected_material = _select_event_summary_material(candidate, event_dict)
        summary = _normalize_event_summary_text(selected_material, min_chars=min_chars, max_chars=max_chars)
        if summary:
            return summary
    return ""
...
def _event_summary_quality(text):
    ...
    if _EVENT_SUMMARY_TARGET_MIN <= length <= _EVENT_SUMMARY_TARGET_MAX:
        length_score = 260 - abs(140 - length)
    elif 60 <= length < _EVENT_SUMMARY_TARGET_MIN:
        length_score = 150 - (_EVENT_SUMMARY_TARGET_MIN - length) * 1.5
    elif _EVENT_SUMMARY_TARGET_MAX < length <= 240:
        length_score = 150 - (length - _EVENT_SUMMARY_TARGET_MAX)
    else:
        length_score = 40
    sentence_score = {
        1: 12,
        ...
        3: 92,
        4: 100,
    }.get(sentence_count, 72 if sentence_count > 4 else 0)
    return length_score + sentence_score + min(cjk_count, 150)
...
def build_event_blueprints(ai_driver, raw_search_results, topic, current_date, time_opt, history_hint="", guidance=""):
    ...
    9. 每条事件必须填写 event_summary：必须使用自然中文新闻导语风格，通常为 3 到 4 个完整句子，建议 100 到 180 个中文字符。第一句交代主体、时间和核心动作，第二句说明产品、功能、政策或业务对象，后续句只补充输入中明确披露的数据、客户、市场、供应链节点及直接影响；信息不足时允许略短，但不得重复标题、解释检索过程、编造事实或直接复制英文摘要。不得使用“公开材料显示”“该线索由某网站披露”“材料没有提供足够细节”“暂不能确认更多参数”“时间线仅记录已披露动作”“进一步看”“补充判断”等免责声明或空泛补句。
    """
