...
POLICY_EVENT_TERMS = [
    "监管", "政策", "regulation", "regulator", "ban", "probe", "investigation", "executive order",
    "tariff", "trade", "sanction", "middle east", "israel", "iran", "gaza", "red sea", "oil"
]
SOCIAL_EVENT_TERMS = ["未成年人", "社交", "adult", "teen", "children", "social media"]
_CJK_RE = re.compile(r"[\u4e00-\u9fff]")
TIMELINE_TARGET_MIN = 9
TIMELINE_HARD_LIMIT = 16
...
def _event_sort_key(event_dict):
    date_text = str(event_dict.get("date", "") or "")
    sort_rank = event_dict.get("_sort_rank", 999)
    if sort_rank in (None, ""):
        sort_rank = 999
    else:
        sort_rank = int(sort_rank)
    match = re.search(r"(\d{2})月(\d{2})日", date_text)
    if match:
        return (
            0,
            int(match.group(1)),
...
def _compact_timeline_title(text, cjk_limit=26, ascii_limit=42):
    cleaned = str(text or "").strip()
    if not cleaned:
        return cleaned
    candidates = []
    ...
    if _CJK_RE.search(cleaned):
        for candidate in candidates:
            if 8 <= len(candidate) <= cjk_limit:
                return candidate
        return cleaned[:cjk_limit]

    for candidate in candidates:
        if 8 <= len(candidate) <= ascii_limit:
            return candidate
    return cleaned[:ascii_limit]
...
def _clean_title_for_timeline(title, topic="", keywords=None):
    ...
    return _compact_timeline_title(cleaned, cjk_limit=26, ascii_limit=42)


def _build_event_candidate_from_result(result, topic=""):
    event_text = _clean_title_for_timeline(result.get("title") or "未命名事件", topic=topic)
    if _looks_broken_event(event_text, topic):
        event_text = _heuristic_localize_event(result.get("title") or "", topic=topic)
    return {
        ...
        "keywords": [],
        "_rank": int(result.get("_timeline_rank", 0) or 0),
        "_priority_score": float(
            result.get("_company_pack_score")
            or result.get("_pack_score")
            or 0.0
        ),
        "_sort_rank": 100 + int(result.get("_timeline_rank", 0) or 0),
    }


def _backfill_events_from_results(events, raw_search_results, topic="", target_min=TIMELINE_TARGET_MIN, hard_limit=TIMELINE_HARD_LIMIT):
    target_size = min(hard_limit, max(int(target_min or 0), len(events or [])))
    if len(events or []) >= target_size:
        return list(events or [])[:hard_limit]

    existing_events = [dict(item) for item in (events or [])]
    existing_source_counts = {}
    for item in existing_events:
        source = str(item.get("source", "") or "").strip().lower()
        if source:
            existing_source_counts[source] = existing_source_counts.get(source, 0) + 1

    candidates = []
    for idx, result in enumerate(raw_search_results or []):
        candidate = _build_event_candidate_from_result(dict(result or {}, _timeline_rank=idx), topic=topic)
        if _looks_broken_event(candidate.get("event", ""), topic):
            continue
    ...
    for idx, candidate in enumerate(candidates):
        ...
        for other in candidates:
            if _event_match_score(candidate, other) >= 0.46:
                support_count += 1
                if other.get("source"):
                    support_sources.add(str(other.get("source")).lower())
        scored_candidates.append(
            {
                "candidate": candidate,
                "score": round(
                    support_count * 1.25
                    + min(len(support_sources), 3) * 0.35
                    + min(float(candidate.get("_priority_score", 0.0) or 0.0), 12.0) / 12.0
                    + max(0.0, 0.24 - idx * 0.02),
                    4,
                ),
            }
        )
    ...
    scored_candidates.sort(
        key=lambda row: (
            row["score"],
            row["candidate"].get("date", ""),
            -int(row["candidate"].get("_rank", 0) or 0),
        ),
        reverse=True,
    )
    ...
    for row in scored_candidates:
        ...
        if any(_event_match_score(candidate, existing["candidate"]) >= 0.62 for existing in collapsed_candidates):
            continue
        collapsed_candidates.append(row)

    for row in collapsed_candidates:
        if len(existing_events) >= target_size:
            break
        ...
        for existing in existing_events:
            if (
                ...
                and candidate.get("source_url") == existing.get("source_url")
            ):
                duplicate = True
                break
            if _event_match_score(candidate, existing) >= 0.78:
                duplicate = True
                break
        if duplicate:
            continue

        source_key = str(candidate.get("source", "") or "").strip().lower()
        if source_key and existing_source_counts.get(source_key, 0) >= 2 and len(existing_events) >= max(4, target_size - 2):
            continue

        existing_events.append(candidate)
        if source_key:
            existing_source_counts[source_key] = existing_source_counts.get(source_key, 0) + 1

    return existing_events[:hard_limit]
...
def _rewrite_titles_with_ai(ai_driver, events, topic):
    ...
    payload = []
    for item in events[:TIMELINE_HARD_LIMIT]:
        payload.append(
            {
...
def _rewrite_event_dicts(ai_driver, events, topic, raw_search_results=None):
    ...
    for event_dict in events:
        ...
        if matched_result and match_score >= 0.34:
...
def _dedupe_finalized_events(events, max_items=TIMELINE_HARD_LIMIT):
    finalized = []
    for event_dict in events:
...
def _limit_overrepresented_categories(events, target_min=TIMELINE_TARGET_MIN, hard_limit=TIMELINE_HARD_LIMIT):
    if not events:
        return []

    caps = {
        "frontier": hard_limit,
        "business": max(4, hard_limit // 3),
        "generic": max(6, hard_limit // 2),
        "legal": 1,
        "policy": 3,
        "social": 1,
    }
...
def _finalize_event_blueprints(events, ai_driver=None, topic="", raw_search_results=None):
    ...
    for event in events or []:
        ...
        for idx, existing in enumerate(deduped_events):
            ...
            score = _event_match_score(event_dict, existing)
            if signature == existing_signature or score >= 0.88:
                duplicate_index = idx
                break
    ...
    diversified_events = _limit_overrepresented_categories(
        deduped_events,
        target_min=TIMELINE_TARGET_MIN,
        hard_limit=TIMELINE_HARD_LIMIT,
    )
    diversified_events = _rewrite_event_dicts(ai_driver, diversified_events, topic, raw_search_results=raw_search_results)
    diversified_events = _dedupe_finalized_events(diversified_events, max_items=TIMELINE_HARD_LIMIT)
    for idx, event_dict in enumerate(diversified_events):
        event_dict.setdefault("_sort_rank", idx)
    diversified_events = _backfill_events_from_results(
        diversified_events,
        raw_search_results,
        topic=topic,
        target_min=TIMELINE_TARGET_MIN,
    ...
    diversified_events = _dedupe_finalized_events(diversified_events, max_items=TIMELINE_HARD_LIMIT)
    diversified_events = sorted(diversified_events, key=_event_sort_key)

    finalized = []
    for event_dict in diversified_events[:TIMELINE_HARD_LIMIT]:
        event_dict.pop("_category", None)
        event_dict.pop("_rank", None)
        event_dict.pop("_priority_score", None)
        event_dict.pop("_sort_rank", None)
...
def _fallback_event_blueprints(raw_search_results, ai_driver=None, topic=""):
    events = []
    for result in raw_search_results[:TIMELINE_HARD_LIMIT]:
...
def build_event_blueprints(ai_driver, raw_search_results, topic, current_date, time_opt, history_hint="", guidance=""):
    ...
    任务与规则：
    1. 先做事件规范化，核心时间线至少保留 7 条；通常应覆盖到 8 到 12 条值得记录的主档事件。如果当天事件确实密集，可放宽到 16 条，最多不超过 16 条，按事件爆出时间从过去到现在排序。
    ...
    8. 优先覆盖产品、AI、芯片、数据中心、供应链、自动驾驶、政策、地缘与市场催化等不同方向；法律、隐私、诉讼、律师、法庭类如果不是当天主线，最多保留 1 条。
    9. 如果【{topic}】本身是政策、宏观或地缘专题，贸易、关税、补贴、制裁、中东局势、能源、外交等就是主线，不要因为它们不适合写成长文就删掉。
    10. 核心时间线不等于深度新闻候选池。即使某条更新后续不一定写成长新闻，只要它是当天有意义的硬件、产品、供应链、地缘或市场进展，也应保留进时间线。
    """
