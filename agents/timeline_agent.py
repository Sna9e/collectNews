...
def _limit_overrepresented_categories(events, target_min=8, hard_limit=16):
    if not events:
        return []
...
def _finalize_event_blueprints(events, ai_driver=None, topic="", raw_search_results=None):
    ...
    for event in events or []:
        ...
        for idx, existing in enumerate(deduped_events):
            ...
            score = _event_match_score(event_dict, existing)
            if signature == existing_signature or score >= 0.84:
                duplicate_index = idx
                break
    ...
    deduped_events = _supplement_events_from_results(
        deduped_events,
        raw_search_results,
        topic=topic,
        target_min=MIN_TIMELINE_EVENT_COUNT,
    )
    ...
    diversified_events = _dedupe_finalized_events(diversified_events, max_items=16)
    diversified_events = _supplement_events_from_results(
        diversified_events,
        raw_search_results,
        topic=topic,
        target_min=MIN_TIMELINE_EVENT_COUNT,
    ...
    finalized = []
    for event_dict in diversified_events[:16]:
...
MIN_TIMELINE_EVENT_COUNT = 8


def _event_dict_from_result(result, topic=""):
...
def _supplement_events_from_results(events, raw_search_results, topic="", target_min=MIN_TIMELINE_EVENT_COUNT):
    supplemented = list(events or [])
    if len(supplemented) >= target_min:
        return supplemented

    for result in raw_search_results or []:
        candidate = _event_dict_from_result(result, topic=topic)
        if _looks_broken_event(candidate.get("event", ""), topic):
            continue

        duplicate = False
        for existing in supplemented:
            same_url = bool(candidate.get("source_url") and candidate.get("source_url") == existing.get("source_url"))
            same_title = _normalize_text(candidate.get("event", "")) == _normalize_text(existing.get("event", ""))
            if same_url or same_title or _event_match_score(candidate, existing) >= 0.94:
                duplicate = True
                break
        if duplicate:
            continue
...
def _fallback_event_blueprints(raw_search_results, ai_driver=None, topic=""):
    events = []
    for result in raw_search_results[:12]:
        event_dict = _event_dict_from_result(result, topic=topic)
        events.append(
            EventDraft(
                date=event_dict["date"],
                source=event_dict["source"],
                event=event_dict["event"],
                source_url=event_dict["source_url"],
                keywords=event_dict["keywords"],
            )
        )
...
def build_event_blueprints(ai_driver, raw_search_results, topic, current_date, time_opt, history_hint="", guidance=""):
    ...
    任务与规则：
        1. 先做事件规范化，至少保留 7 条；常规保留 8 到 12 条真正最值得后续展开的核心事件主档；如果当天事件确实密集，可放宽到 16 条，按事件爆出时间从过去到现在排序。
