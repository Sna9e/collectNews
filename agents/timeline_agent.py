...
def _limit_overrepresented_categories(events, target_min=6, hard_limit=20):
    if not events:
        return []
...
def _finalize_event_blueprints(events):
    ...
    diversified_events = _limit_overrepresented_categories(deduped_events, target_min=6, hard_limit=20)

    finalized = []
    for event_dict in diversified_events[:20]:
