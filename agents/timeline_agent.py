...
class EventDraft(BaseModel):
    ...
    event: str = Field(description="18到28字的一句话核心短讯，既要简洁，也要保留主体和动作")
...
class TimelineTitleDraft(BaseModel):
    event: str = Field(description="中文短讯标题，12到28字，突出主体、动作和关键信息")
...
def _rewrite_titles_with_ai(ai_driver, events, topic):
    ...
    2. event 必须是中文核心时间线短讯，优先 14 到 26 个字，最长不超过 30 个字。
...
def _heuristic_localize_event(event_text, topic="", keywords=None):
    ...
    if topic_hint:
        ...
        return fallback[:48] if fallback and not _looks_broken_event(fallback, topic) else f"{topic_hint}最新进展"
    if keyword_hint:
...
def build_event_blueprints(ai_driver, raw_search_results, topic, current_date, time_opt, history_hint="", guidance=""):
    ...
    5. event 保持一句话短讯风格，优先 18 到 28 个字；要写出主体和动作，不要只剩过短名词短语；keywords 提炼 3 到 6 个关键词；source_url 必须尽量填写对应原始 URL。
    6. 只保留要求时间窗口内的近期事件。
