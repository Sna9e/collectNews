...
_SUMMARY_LOW_SIGNAL_CN_RE = re.compile(
    r"(微信扫一扫|微信扫一扫关注|打开微信扫一扫|扫一扫|扫码下载|扫码下载查看详情|扫码查看详情|"
    r"手机有色|掌上有色|今日有色|app-smm|官方出品|热点资讯|深度解析|独家调研|服务时间|"
    r"分享至|关注公众号|下载查看更多|查看详情|我要入驻|媒体品牌|企业服务|政府服务|投资人服务|"
    r"创业者服务|创投平台|ai测评网|上海有色网|有色网)",
    re.IGNORECASE,
)
...
def _contains_summary_noise(text):
    return bool(_SUMMARY_LOW_SIGNAL_RE.search(str(text or ""))) or bool(_SUMMARY_LOW_SIGNAL_CN_RE.search(str(text or "")))


def _clean_summary_fragment(text):
    cleaned = str(text or "").replace("\r", "\n")
    cleaned = _SUMMARY_LOW_SIGNAL_RE.sub(" ", cleaned)
    cleaned = _SUMMARY_LOW_SIGNAL_CN_RE.sub(" ", cleaned)
...
def _sanitize_generated_summary(summary_text):
    ...
    for line in raw_lines:
        ...
        if _SUMMARY_LOW_SIGNAL_RE.search(lower):
            continue
        if _SUMMARY_LOW_SIGNAL_CN_RE.search(cleaned.replace(" ", "")):
            continue
        if len(cleaned) <= 2:
            continue
        if (
...
def _build_fallback_summary(topic, blueprint, supporting_results):
    ...
    primary_fact = _trim_text(snippet, 220) if snippet_score >= 0.52 and not _contains_summary_noise(snippet) and not _contains_meta_analysis(snippet) else ""
    secondary_fact = _trim_text(secondary_snippet or secondary_title, 180) if secondary and float(secondary.get("_snippet_score", 0.0) or 0.0) >= 0.45 and not _contains_summary_noise(secondary_snippet) and not _contains_meta_analysis(secondary_snippet) else _trim_text(secondary_title, 68)
    tertiary_fact = _trim_text(tertiary_snippet or tertiary_title, 160) if tertiary and float(tertiary.get("_snippet_score", 0.0) or 0.0) >= 0.45 and not _contains_summary_noise(tertiary_snippet) and not _contains_meta_analysis(tertiary_snippet) else _trim_text(tertiary_title, 60)
    ...
    if primary_fact or secondary_fact or tertiary_fact:
        ...
        if secondary_title or secondary_snippet:
            extra_detail = _trim_text(secondary_snippet or secondary_title, 220)
            if extra_detail and not _contains_summary_noise(extra_detail) and not _contains_meta_analysis(extra_detail):
                fallback_detail += f" 补充来源提到：{extra_detail}"
        parts.append(f"【深度细节/数据支撑】\n{fallback_detail}")
    parts.append(
...
def _collect_supporting_results(blueprint, raw_search_results, limit=3):
    ...
    for result in raw_search_results or []:
        ...
        snippet_text = _clean_summary_fragment(result.get("content", "") or "")
        if _contains_summary_noise(snippet_text) or _contains_meta_analysis(snippet_text):
            snippet_score = 0.0
        else:
            snippet_score = _text_match_score(snippet_text, blueprint)
        score = round(title_score * 0.68 + snippet_score * 0.32, 4)
        if score >= 0.26 and max(title_score, snippet_score) >= 0.20:
            enriched = dict(result)
            enriched["content"] = snippet_text
...
def _expand_short_summary(summary_text, topic, blueprint, supporting_results):
    ...
    for result in supporting_results[:3]:
        ...
        if snippet_score >= 0.50:
            candidate = _trim_text(_clean_summary_fragment(result.get("content") or ""), 220)
        ...
        if not candidate:
            continue
        if _contains_summary_noise(candidate):
            continue
...
def _should_rebuild_summary(summary_text, news_item, blueprint):
    ...
    if _contains_meta_analysis(summary):
        return True
    if _contains_summary_noise(summary):
        return True
