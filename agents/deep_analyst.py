...
def map_reduce_analysis(
    ...
    if source_mode == "search_summary_fallback":
        detail_prompt = (
            "要求每条新闻总中文篇幅不少于 350 字，建议控制在 320 到 450 字。"
            "【事件核心】至少 100 字，【深度细节/数据支撑】至少 150 字，【行业深远影响】至少 80 字。"
            "只能基于现有搜索摘要做事实概括和行业判断，不得伪造原文直接引语、微观动作或超细颗粒数据。"
        )
    elif source_mode == "mixed_fallback":
        detail_prompt = (
            "要求每条新闻总中文篇幅不少于 350 字，建议控制在 360 到 560 字。"
            "【事件核心】至少 100 字，【深度细节/数据支撑】至少 160 字，【行业深远影响】至少 90 字。"
            "可以结合全文和网页抽取做分析，但对直接引语、超细颗粒数字和过度确定的因果判断要保守表达。"
        )
    elif "24" in time_opt:
        detail_prompt = (
            "要求每条新闻总中文篇幅不少于 350 字，建议控制在 360 到 520 字。"
            "【事件核心】至少 100 字，【深度细节/数据支撑】至少 160 字，【行业深远影响】至少 90 字。"
            "优先保留能支撑判断的具体数字、核心动作、原话要点和业务影响；不要为了控制篇幅而把摘要压成过短版本。"
        )
    else:
        detail_prompt = (
            "要求每条新闻总中文篇幅不少于 350 字，建议控制在 360 到 500 字。"
            "【事件核心】至少 100 字，【深度细节/数据支撑】至少 160 字，【行业深远影响】至少 90 字。"
