...
_SUMMARY_NOISY_FOLLOWUP_RE = re.compile(
    r"^(进一步看，.*(20\d{2}[-年/]\d{1,2}[-月/]\d{1,2}).*)$|^(补充判断：围绕.*)$",
    re.IGNORECASE,
)
...
def _sanitize_generated_summary(summary_text):
    ...
    for line in raw_lines:
        ...
        if _SUMMARY_NOISY_FOLLOWUP_RE.search(cleaned):
            continue
        if cleaned.startswith("进一步看，") and len(re.findall(r"20\d{2}", cleaned)) >= 2:
            continue
        if len(cleaned) <= 2:
            continue
        if (
...
def _build_fallback_summary(topic, blueprint, supporting_results):
    ...
    if snippet:
        ...
        if secondary_title or secondary_snippet:
            detail_lines.append(f"进一步看，{_trim_text(secondary_snippet or secondary_title, 360)}")
        if tertiary_title or tertiary_snippet:
            detail_lines.append(f"同时，材料还提到：{_trim_text(tertiary_snippet or tertiary_title, 300)}")
...
def _expand_short_summary(summary_text, topic, blueprint, supporting_results):
    ...
    for result in supporting_results[:3]:
        candidate = _trim_text(result.get("content") or result.get("title") or "", 320)
        if not candidate:
            continue
    ...
    if extra_lines:
        summary += "\n进一步看，" + "；".join(extra_lines[:2]).rstrip("；;。") + "。"
...
def map_reduce_analysis(
    ...
    if has_event_blueprints and source_mode == "consumer_daily_verified_events":
    ...
    minimum_summary_chars = 320 if source_mode in {"consumer_daily_digest", "consumer_daily_verified_events"} else 380

    consumer_daily_rules = ""
    if source_mode in {"consumer_daily_digest", "consumer_daily_verified_events"}:
        consumer_daily_rules = """
        【频道三专用红线】：
        ...
        6. 不要输出“进一步阅读”“相关推荐”“大家都在看”“热门文章”“补充判断：围绕……”等网页噪声或空泛补丁句。
        7. 对 verified events，正式正文只允许写 confirmed / likely；likely 必须写“待核实”，不得把 weak、rumor、stale、rejected 当新闻扩写。
        8. 正文中必须出现“独立来源”和“主要来源”的证据链信息，但不要输出内部 event_id。
        9. 每个专题原则上输出 3-5 条主新闻；如果输入 confirmed+likely 不足 3 条，不得编造补足，只说明“待跟踪线索”由系统在 PPT 中单独列示。
        """

    if source_mode == "consumer_daily_verified_events":
        detail_prompt = (
