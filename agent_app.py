...
class NewsItem(BaseModel):
    ...
    date_check: str = Field(description="严格核实新闻发生的真实日期，格式 YYYY-MM-DD。若判断为旧闻，此条作废！")
...
def search_web(query, sites_text, timelimit, max_results=15, tavily_key=""):
    ...
    # ======== 通道 1：Tavily API (专属 News 模式，物理级时间限制) ========
    if tavily_key:
        try:
            ...
            payload = {
                ...
                "topic": "news",  # 强制启用新闻专属引擎，自动屏蔽老文章
                "max_results": max_results
            }
            if sites: payload["include_domains"] = sites
            
            # 精准物理时间控制（为防止时区误差，24h设为2天缓冲）
            if timelimit == "d":
                payload["days"] = 2 
            elif timelimit == "w":
                payload["days"] = 7
...
# 🔴 灵魂级时间锁：用“极度偏执”的 Prompt 逼迫大模型严查时间
def map_reduce_analysis(ai_driver, topic, full_text, current_date, time_opt):
    ...
    def process_single_doc(doc):
        ...
        用户要求的绝对时间范围是：【{time_opt}】。

        当前任务：从以下文本中提取关于【{topic}】的新闻情报。
        【生死红线（违反任何一条，必须返回空列表）】：
        1. 极度严格的时间审查：你必须像侦探一样寻找文章中的时间线索（如具体日期、“昨天”、“X小时前”）。如果发现该事件发生时间明显早于【{time_opt}】之前（例如几个月前、去年的旧闻），**绝对不要提取，直接丢弃！**
        2. 绝对主角原则：【{topic}】必须是事件核心，只是一笔带过的边角料直接丢弃！
        
        如果没有完全符合上述严苛条件的内容，请毫不犹豫地返回 `{{"news": []}}`。
        文本内容：
        {doc.page_content}
        """
    ...
    reduce_prompt = f"""
        【全局时间锚点】：今天是 **{current_date}**。
        你是极其严苛的科技媒体总编。以下是初筛数据。
        
        【终极审判任务】：
        1. 终极时间清洗：再次核实！任何发生在【{time_opt}】之前的陈年旧闻，全部无情删掉！宁可最后一条不留，也绝不允许一篇旧闻混入研报！
        2. 合并同类项并去重。
        3. 扩写：不少于 400 字（必须包含：事件核心、深度细节、行业影响）。
        4. 纯中文专业排版输出。
        5. 按重要性降序，最多保留最硬核的 6 条。
        数据：{combined_json}
    """
...
def generate_word(data, filename, model_name):
    ...
    for section in data:
        ...
        if not section['data']:
            doc.add_paragraph("在指定时间范围内，未发现符合标准的极具时效性的重大情报。")
            continue
        for news in section['data']:
            doc.add_heading(news.title, level=2)
            p = doc.add_paragraph()
            run = p.add_run(f"来源: {news.source} | 真实时间: {news.date_check} | 热度: {'⭐'*news.importance}")
...
if btn:
    if not api_key:
        ...
        for topic in topics:
            ...
            if full_text_data:
                st.write(f"🧠 成功提取 {valid_count} 个纯净网页。DeepSeek 正在执行【无情的时间清洗】（约耗时10-30秒）...")
                
                with st.spinner("AI 正在剔除旧闻，提炼最新精华..."):
                ...
                if final_news_list:
                    ...
                    st.success(f"✅ 【{topic}】分析完毕！已为您锁定 {len(final_news_list)} 条纯正的新鲜情报。")
                else:
        ...
        if all_data:
            ...
            st.balloons()
            st.success("🎉 全链条任务执行完毕！没有任何旧闻可以逃过法眼！")
            with open(path, "rb") as f:
                st.download_button("📥 立即下载中文深度研报 (Word)", f, file_name=path, type="primary")
        else:
            st.error(f"❌ 任务结束。在严格的【{time_opt}】时效约束下，所有关键词均未产生有效的大事件情报。")
