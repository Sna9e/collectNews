...
def test_exa_long_windows_use_explicit_publication_boundaries():
    captured = {"news": [], "patent": [], "paper": []}

    def capture_search(query, sites_text, timelimit, **kwargs):
        query_lower = query.lower()
        item_type = "patent" if "patent" in query_lower or "专利" in query else (
            "paper" if "paper" in query_lower or "论文" in query else "news"
        )
        captured[item_type].append(dict(kwargs.get("exa_settings") or {}))
        return _fake_search(query, sites_text, timelimit, **kwargs)
    ...
    assert captured["news"] and captured["patent"] and captured["paper"]
    assert captured["news"][0]["category"] == "news"
    assert captured["patent"][0]["category"] == ""
    assert captured["paper"][0]["category"] == ""
    assert captured["patent"][0]["start_published_date"].startswith("2025-06-22")
    assert captured["paper"][0]["start_published_date"].startswith("2023-06-23")
...
def run_all():
    tests = [
        ...
        test_exa_long_windows_use_explicit_publication_boundaries,
    ]
    for test in tests:
        test()
