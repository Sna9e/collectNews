...
def test_domain_normalization_and_subdomain_matching_are_safe():
    ...
    assert normalize_domain_rule("http://www.xix.ai/?lang=en") == "xix.ai"
...
def test_builtin_and_manual_domain_rules_block_results():
    ...
    builtin_domains = {rule["domain"] for rule in builtins}
    assert required_hard_blocks.issubset(builtin_domains)
    for domain in required_hard_blocks:
        assert evaluate_search_result(_result(f"www.{domain}"))["blocked"] is True

    persistent = load_user_source_blocklist()
    assert {
        "bitrss.com",
        "dev.to",
        "vocus.cc",
        ...
        "villadaba.com",
        "xix.ai",
    }.issubset(set(persistent["domains"]))
...
def test_provider_payloads_receive_effective_exclusions():
    ...
    assert "36kr.com" in captured["excludeDomains"]
    assert "thevergetoday.pages.dev" in captured["excludeDomains"]


def test_36kr_is_not_configured_as_a_priority_source():
    files = [
        ROOT / "tools" / "search_engine.py",
        ROOT / "tools" / "intelligence_packs.py",
        ROOT / "tools" / "consumer_topic_query_packs.py",
        ROOT / "tools" / "source_registry.json",
    ]
    for path in files:
        assert "36kr.com" not in path.read_text(encoding="utf-8").lower(), path
...
if __name__ == "__main__":
    tests = [
        ...
        test_36kr_is_not_configured_as_a_priority_source,
