...
def test_exa_expansion_can_reach_three_main_events():
    ...
    def expansion_search(query, _topic_pack, _window=None):
        if "Rokid" not in query and "XREAL" not in query and "Meta" not in query:
            return []
        return [
            _result("Rokid 智能眼镜今日公布参数升级", "rokid.com", "Rokid今日发布智能眼镜参数升级，涉及光波导、显示模组和AI识别。公司同步说明新功能将通过系统更新覆盖在售设备。", source="Rokid官网", path="/rokid"),
            _result("XREAL AR眼镜今日更新显示模组方案", "xreal.com", "XREAL今日发布AR眼镜显示模组更新，涉及Micro OLED和近眼显示体验。新方案用于改善画面亮度与佩戴场景下的显示稳定性。", source="XREAL官网", path="/xreal"),
            _result("Meta Quest VR头显今日公布系统更新", "meta.com", "Meta今日发布Quest VR头显系统更新，重点是空间计算、手势追踪和开发者功能。更新将分批推送，并调整应用交互与开发接口。", source="Meta官网", path="/quest"),
        ]
