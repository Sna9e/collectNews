...
def test_report_writers_filter_builtin_and_manual_blocked_sources():
    rows = [
        _row("Trusted source clue", "connector", 82, source_url="https://molex.com/pwg"),
        _row("Built-in blocked clue", "connector", 90, source_url="https://newsbreak.com/pwg"),
        _row("Manual blocked clue", "connector", 88, source_url="https://feeds.robot.example/pwg"),
    ]
    with tempfile.TemporaryDirectory() as tmpdir:
        output = write_daily_brief(
            rows,
            report_date=REPORT_DATE,
            output_dir=tmpdir,
            generated_at=GENERATED_AT,
            blocked_domains="robot.example",
        )
        text = output.read_text(encoding="utf-8")
        assert "Trusted source clue" in text
        assert "Built-in blocked clue" not in text
        assert "Manual blocked clue" not in text
...
def run_all():
    tests = [
        ...
        test_report_writers_filter_builtin_and_manual_blocked_sources,
