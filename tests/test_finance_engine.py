...
def test_yahoo_chart_normalization_generates_market_chart():
    ...
    assert payload["trailing_pe"] == 28.4
    assert payload["forward_pe"] == 24.8
    assert payload["price_to_book"] == 7.2
    assert payload["earnings_yield_pct"] == 3.52
    assert payload["valuation_assessment"] == "PE绝对值处于中等区间"
    assert payload["price_assessment"] in {"短期偏强", "震荡中性", "短期偏弱"}
    assert payload["research_stance"] in {"积极观察", "中性观察", "谨慎观察"}
    assert "不构成个性化投资建议" in payload["analysis_disclaimer"]
...
def test_finance_analysis_does_not_invent_valuation_or_target_price():
    if finance_engine.pd is None:
        return "SKIP pandas unavailable"
    frame = finance_engine.pd.DataFrame(
        {
            "Open": list(range(10, 31)),
            "High": list(range(11, 32)),
            "Low": list(range(9, 30)),
            "Close": list(range(10, 31)),
            "Volume": [1000] * 21,
        },
        index=finance_engine.pd.date_range("2026-07-01", periods=21, freq="B"),
    )
    analysis = finance_engine._build_finance_analysis(
        frame,
        {"low_52w": 8, "high_52w": 32},
        current_price=30,
    )
    assert analysis["trailing_pe"] is None
    assert analysis["valuation_assessment"] == "估值数据不足"
    assert analysis["research_stance"] in {"中性观察", "谨慎观察"}
    assert "target_price" not in analysis
    assert "目标价" in analysis["analysis_disclaimer"]
...
if __name__ == "__main__":
    tests = [
        ...
        test_finance_analysis_does_not_invent_valuation_or_target_price,
    ]
    for test in tests:
        test()
