...
def test_normal_search_content_builds_complete_summary():
    ...
    assert 100 <= len(summary) <= 180
    assert 3 <= _sentence_count(summary) <= 4


def test_summary_selects_event_specific_sentences_instead_of_first_sentences():
    result = {
        "content": (
            "本周全球科技市场交易活跃，多家公司公布了常规业务动态。"
            "行业活动还覆盖消费电子、云计算和软件服务等多个方向。"
            "英伟达宣布 Vera Rubin 平台进入全面生产，面向下一代 AI 工厂提供 CPU、GPU 和网络组件。"
            "服务器厂商将围绕新平台推进机架、液冷和网络系统验证。"
            "云服务客户的采购计划将随产品切换调整，上游板卡与散热供应链也需同步准备。"
        )
    }
    summary = _build_event_summary_from_result(
        result,
        {"event": "英伟达 Vera Rubin 投产", "keywords": ["Vera Rubin", "AI 工厂", "服务器"]},
    )
    assert "英伟达宣布 Vera Rubin 平台进入全面生产" in summary
    assert "全球科技市场交易活跃" not in summary
    assert "行业活动还覆盖" not in summary
    ...
    return [
        {
            ...
            "event_summary": "苹果在6月发布端侧AI计划，重点面向iPhone和Mac提供本地模型能力。该计划覆盖系统体验、开发者接口和设备端推理场景。报道提到芯片算力、隐私处理和应用适配会成为落地重点。该动作将影响系统升级节奏，并促使供应链和开发者调整测试与适配安排。",
            "appears_in_later_news": True,
            "matched_news_title": "苹果端侧AI计划推动设备体验升级",
            "matched_news_index": 1,
            "matched_news_importance": 5,
            "highlight_level": "key",
        ...
        },
        {
            ...
            "event_summary": "供应链消息称苹果调整新一代iPhone备货节奏，涉及摄像头、显示模组和主板环节。现有信息指向部分零部件订单节奏变化，但未披露具体数量。该变化会影响上游厂商排产、库存和交付安排。备货调整有助于匹配新品发布前后的需求预期，并减少发布初期供需错配风险。",
        },
        {
            "date": "06月02日",
            "event": "Mac芯片路线更新",
            ...
            "event_summary": "苹果更新Mac芯片路线，新平台聚焦能效、图形性能和AI推理能力。材料提到高端笔记本和桌面设备会优先采用该平台。该路线会改变开发者对本地推理、图形渲染和续航的优化重点，并影响专业软件的验证计划。新芯片也将强化Mac硬件差异化，后续节点取决于具体机型发布安排。",
...
def test_ppt_timeline_summaries_pagination_and_link_prompt():
    ...
    for text in timeline_slides:
        assert len(re.findall(r"(?m)^\s*(?:◆\s*)?(?:★\s*|●\s*)?\[\d{2}月\d{2}日\]", text)) <= 3
    ...
    assert "↳ 详见后文：详细新闻 1《苹果端侧AI计划推动设备体验升级》" in all_timeline_text
    ...
    for summary in normal_summaries:
        ...
        assert by_text[summary]["font_size"] == Pt(11)
...
class FakeAI:
    ...
    def analyze_structural(self, prompt, structure_class):
        if structure_class.__name__ == "EventBlueprintReport":
            ...
            assert "3 到 4 个完整句子" in prompt
            assert "100 到 180" in prompt
...
def run_all():
    tests = [
        ...
        test_summary_selects_event_specific_sentences_instead_of_first_sentences,
