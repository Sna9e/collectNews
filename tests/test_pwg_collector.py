...
def test_collect_pwg_daily_scan_reports_empty_filtered_result():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert payload["raw_result_count"] == 0
        assert payload["kept_count"] == 0
        assert payload["warnings"]
        assert "未返回原始结果" in payload["warnings"][0]
...
def run_all():
    tests = [
        ...
        test_collect_pwg_daily_scan_reports_empty_filtered_result,
    ]
    for test in tests:
        test()
