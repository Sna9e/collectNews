...
def test_channel3_uses_five_major_news_topic_packs():
    packs = topic_packs.get_channel3_major_news_topic_query_packs()
    ids = [pack.topic_id for pack in packs]
    assert ids == [
        "consumer_phone",
        "ar_vr_ai_glasses",
    ...
    assert "foldable_display_supply_chain" not in ids
...
def test_channel3_page_date_review_overrides_wrong_provider_timestamp():
    pack = _pack("consumer_phone")
    provider_fresh_but_page_old = _result(
        "华为手机旧闻被搜索供应商标成今日",
        "example.com",
        "华为此前发布手机产品，网页正文为历史回顾。",
    )

    def fake_page_date_fetcher(_url):
        return {"published_at": "2026-04-01T08:00:00+08:00", "source_key": "page_test"}

    filtered, stats, warnings = topic_packs.filter_consumer_results_by_freshness(
        [provider_fresh_but_page_old],
        pack,
        "7d",
        TARGET_DT,
    ...
    assert stats["window_label"].startswith("最近7天")
    assert warnings
...
def test_agent_app_channel3_pipeline_static_contract():
    ...
    tail = text.index("with tab4:", start)
    block = text[start:tail]
    assert "get_channel3_major_news_topic_query_packs()" in block
    assert "collect_consumer_topic_search_results(" in block
    assert "filter_consumer_results_by_freshness(" in block
    assert "build_verified_digest_news_items(" in block
    assert "consumer_daily_verified_digest" in block
    assert "enable_llm_enhancement" in block
...
def run_all():
    tests = [
        ...
        test_channel3_uses_five_major_news_topic_packs,
        ...
        test_channel3_page_date_review_overrides_wrong_provider_timestamp,
