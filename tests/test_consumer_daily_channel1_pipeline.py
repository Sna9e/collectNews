...
def test_five_topic_packs_load():
...
def test_each_topic_pack_generates_at_least_ten_queries():
    for pack in topic_packs.get_all_consumer_topic_query_packs():
        ...
        assert len(queries) >= 10, pack.topic_id
...
def test_event_master_can_be_generated_from_topic_results():
    ...
    rows = [
        _result("小鹏 OTA 推送高阶智驾更新", "autohome.com.cn", "小鹏 OTA 智驾 城市NOA 发布"),
        _result("比亚迪发布电池新技术", "gasgoo.com", "比亚迪 电池 800V 技术 发布"),
    ]
...
def test_map_reduce_fallback_outputs_structured_event_news():
    ...
    rows = [_result("华为新机发布 AI 功能升级", "ithome.com", "华为 手机 新品 参数 AI 功能 发布")]
